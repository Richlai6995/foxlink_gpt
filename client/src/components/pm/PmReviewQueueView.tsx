/**
 * PmReviewQueueView — 採購員 v2 prompt 審核 UI
 *
 * 從原 PmReviewQueuePage 抽出,支援兩種使用情境:
 *   1. embedded=false(預設):standalone /pm/review 用,有 page header(返回 / Webex 訂閱 / 手動跑 / refresh)
 *   2. embedded=true:嵌入 /pm/briefing tab 用,只顯示 list + detail,Webex 訂閱 / refresh 仍可用但縮成 inline buttons
 *
 * /pm/review 仍 deep link OK(舊 sidebar 連結 / Webex Bot / 通知信仍可用)
 */
...
export default function PmReviewQueueView({ embedded = false, onPendingCountChange }: Props) {
  ...
  const fetchList = async () => {
    ...
    try {
      ...
      setList(arr)
      // 同步 pending count(給 parent / sidebar badge 用)
      if (onPendingCountChange) {
        if (filter === 'pending') onPendingCountChange(arr.length)
        else {
          // 重撈一次 pending 拿正確 count
          api.get('/pm/review/queue', { params: { status: 'pending' } })
            .then(r => onPendingCountChange((r.data || []).length))
            .catch(() => {})
        }
      }
    } catch (err: any) {
  ...
  if (denied) {
    return (
      <div className="flex flex-col items-center justify-center h-full bg-slate-50 text-slate-500 gap-3 py-16">
  ...
  return (
    <div className={containerClass}>
      {/* Header — embedded 時改成輕量 inline bar */}
      {embedded ? (
        <div className="flex items-center gap-2 px-4 py-2 border-b bg-white">
          <Sparkles size={14} className="text-amber-500" />
          <span className="text-sm font-medium text-slate-700">Prompt 審核 — 採購員審 LLM 自動產生的 v2 prompt</span>
          <div className="ml-auto flex items-center gap-1">
            <button
              onClick={() => setShowSubsModal(true)}
              className="flex items-center gap-1 px-2 py-1 text-xs rounded border border-blue-300 text-blue-700 hover:bg-blue-50"
              title="Webex 推送訂閱"
            ><Bell size={12} /> Webex 訂閱</button>
            {isAdmin && (
              <button
                onClick={runSelfImprove} disabled={runningSelfImprove}
                className="flex items-center gap-1 px-2 py-1 text-xs rounded border border-amber-300 text-amber-700 hover:bg-amber-50 disabled:opacity-50"
              >
                {runningSelfImprove ? <Loader2 size={12} className="animate-spin" /> : <RefreshCw size={12} />}
                跑 self-improve
              </button>
            )}
            <button onClick={fetchList} className="p-1 text-slate-500 hover:bg-slate-100 rounded"><RefreshCw size={14} /></button>
          </div>
        </div>
      ) : (
...
interface SubItem {
  id: number; kind: string; schedule_hhmm: string; is_active: number;
  last_sent_at: string | null; last_sent_date: string | null; created_at: string;
}
...
function WebexSubsModal({ onClose }: { onClose: () => void }) {
  ...
  const fetchSubs = async () => {
    setLoading(true)
    try { const { data } = await api.get('/pm/subscriptions'); setList(Array.isArray(data) ? data : []) }
    finally { setLoading(false) }
  }
  useEffect(() => { fetchSubs() }, [])
  ...
  const add = async () => {
    ...
    setBusy(true)
    try { await api.post('/pm/subscriptions', { kind: 'daily_snapshot', schedule_hhmm: newHhmm }); await fetchSubs() }
    catch (e: any) { alert(e?.response?.data?.error || String(e)) }
    finally { setBusy(false) }
  }
  const toggle = async (s: SubItem) => { await api.patch(`/pm/subscriptions/${s.id}`, { is_active: s.is_active ? 0 : 1 }); await fetchSubs() }
  const remove = async (id: number) => {
    if (!window.confirm('刪除此訂閱?')) return
    await api.delete(`/pm/subscriptions/${id}`); await fetchSubs()
  }
    ...
    <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50">
      <div className="bg-white rounded-lg w-[560px] max-h-[80vh] flex flex-col">
        <div className="flex items-center justify-between px-5 py-3 border-b">
          <span className="font-medium text-sm flex items-center gap-2"><Bell size={16} className="text-blue-500" /> Webex 推送訂閱</span>
        ...
        <div className="flex-1 overflow-auto p-5 space-y-4">
          ...
          <div className="border border-slate-200 rounded p-3 space-y-2">
            ...
            <div className="flex items-center gap-2">
              ...
              <input type="time" value={newHhmm} onChange={(e) => setNewHhmm(e.target.value)} className="border border-slate-200 rounded px-2 py-1 text-sm" />
              <button onClick={add} disabled={busy} className="ml-auto flex items-center gap-1 px-3 py-1 text-sm rounded bg-blue-600 text-white hover:bg-blue-700 disabled:opacity-50">
                <Plus size={12} /> 新增
              </button>
            </div>
          </div>
          {loading ? (
            <div className="text-center text-slate-400 text-sm py-4 flex items-center justify-center gap-2"><Loader2 size={14} className="animate-spin" /> 載入中…</div>
          ) : list.length === 0 ? (
            <div className="text-center text-slate-400 text-sm py-4 border border-dashed rounded">尚無訂閱</div>
          ) : (
            <div className="space-y-1">
              {list.map(s => (
                <div key={s.id} className={`flex items-center gap-2 text-xs border rounded px-3 py-2 ${s.is_active ? 'bg-white border-slate-200' : 'bg-slate-50 border-slate-100 opacity-60'}`}>
                  <Bell size={14} className="text-blue-400" />
                  <div className="flex-1">
                    <div className="font-medium text-slate-800">{s.kind}</div>
                    <div className="text-slate-500 text-[11px]">每日 {s.schedule_hhmm}{s.last_sent_at && ` · 上次:${s.last_sent_at}`}</div>
                  </div>
                  <button onClick={() => toggle(s)} className={`px-2 py-0.5 rounded text-[10px] ${s.is_active ? 'bg-emerald-100 text-emerald-700' : 'bg-slate-200 text-slate-600'}`}>
                    {s.is_active ? '啟用中' : '已停用'}
                  </button>
                  <button onClick={() => remove(s.id)} className="text-red-400 hover:text-red-600"><Trash2 size={12} /></button>
                </div>
              ))}
            </div>
          )}
...
function DetailPanel({
  ...
  decideBusy, decideComment, setDecideComment, onApprove, onReject,
}: {
  item: QueueDetail
    ...
    <div className="p-6 max-w-5xl mx-auto space-y-4">
      <div className="flex items-center gap-3 border-b pb-4">
        ...
        <div className="ml-auto flex items-center gap-1 text-xs">
          <button onClick={() => setShowCompare('side')} className={`px-2 py-1 rounded ${showCompare === 'side' ? 'bg-slate-800 text-white' : 'text-slate-500 hover:bg-slate-100'}`}>並列</button>
          <button onClick={() => setShowCompare('diff')} className={`px-2 py-1 rounded flex items-center gap-1 ${showCompare === 'diff' ? 'bg-slate-800 text-white' : 'text-slate-500 hover:bg-slate-100'}`}>
            ...
            <div className="mt-2 text-xs">
              ...
              <ul className="space-y-0.5 font-mono text-[11px]">
                {evalParsed.sample.map((s: any, i: number) => (
                  <li key={i}>{s.metal} @ {s.target_date} — pred {s.predicted}, actual {s.actual}, 誤差 {Number(s.pct_error || 0).toFixed(2)}%</li>
                ))}
              </ul>
            </div>
          )}
      ...
      {item.status === 'pending' && (
        <div className="border-t pt-4 mt-4 space-y-3">
          ...
          <div className="flex gap-2 justify-end">
            <button onClick={onReject} disabled={decideBusy} className="flex items-center gap-1.5 px-4 py-2 text-sm rounded border border-red-300 text-red-700 hover:bg-red-50 disabled:opacity-50">
              <XCircle size={14} /> 駁回
            </button>
            <button onClick={onApprove} disabled={decideBusy} className="flex items-center gap-1.5 px-4 py-2 text-sm rounded bg-emerald-600 text-white hover:bg-emerald-700 disabled:opacity-50">
