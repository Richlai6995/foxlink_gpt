 ...
 *  - 顯示當前 Hex + RGB，可一鍵複製
 *  - 點擊外部關閉
 *  - Popover 用 Portal 掛到 body，完全不受父容器 overflow 裁切
 */
...
const SPECTRUM_ROWS: string[][] = [
  HUES.map(h => hslToHex(h, 100, 93)),
  HUES.map(h => hslToHex(h, 100, 82)),
  HUES.map(h => hslToHex(h, 100, 70)),
  HUES.map(h => hslToHex(h, 100, 58)),
  HUES.map(h => hslToHex(h, 100, 46)),
  HUES.map(h => hslToHex(h, 100, 34)),
  HUES.map(h => hslToHex(h, 100, 22)),
  HUES.map(h => hslToHex(h, 65,  55)),
  HUES.map(h => hslToHex(h, 35,  78)),
]
...
// Popover 寬度（px），需與 style.width 一致
const POPOVER_W = 348
...
export default function ColorPickerInput({ value, onChange, title, size = 'sm' }: Props) {
  ...
  const calcPos = useCallback(() => {
    ...
    const POPOVER_H = 320  // 估算高度，夠用
    const spaceBelow = window.innerHeight - rect.bottom
    const openUp = spaceBelow < POPOVER_H && rect.top > POPOVER_H
  ...
  useEffect(() => {
    ...
    const handler = (e: MouseEvent) => {
      ...
      if (
        swatchRef.current && !swatchRef.current.contains(t) &&
  ...
  const popover = open && createPortal(
    ...
    >
      ...
      {tab === 'spectrum' ? (
        <div>
          <div className="grid gap-px" style={{ gridTemplateColumns: `repeat(${HUE_COUNT}, 1fr)` }}>
            {SPECTRUM_ROWS.flat().map((c, i) => (
              <button key={i} type="button" title={c.toUpperCase()}
                onClick={() => selectColor(c)} className={spectrumCls(c)}
                style={{ backgroundColor: c }} />
            ))}
          </div>
          <div className="grid gap-px mt-1.5" style={{ gridTemplateColumns: `repeat(${HUE_COUNT}, 1fr)` }}>
            {GRAYS.map((c, i) => (
              <button key={i} type="button" title={c.toUpperCase()}
                onClick={() => selectColor(c)} className={spectrumCls(c)}
                style={{ backgroundColor: c }} />
        ...
        <div className="grid grid-cols-8 gap-0.5">
          {STANDARD_COLORS.map(c => (
            <button key={c} type="button" title={c.toUpperCase()}
              onClick={() => selectColor(c)} className={standardCls(c)}
              style={{ backgroundColor: c }} />
          ))}
        </div>
      )}
      ...
      <div className="flex items-center gap-2 mt-2.5 mb-2 bg-gray-50 rounded-lg p-1.5">
        <div className="w-8 h-8 rounded-md border border-gray-200 flex-shrink-0 shadow-sm"
      ...
      <div className="flex items-center gap-1">
        ...
        <input
          ...
          placeholder="RRGGBB"
        />
        <button type="button" onClick={applyHexInput}
