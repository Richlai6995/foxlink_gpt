...
export default function UserManagement() {
  ...
  const openEdit = (u: User) => {
    ...
    setForm({
      ...
      is_pipeline_admin: u2.is_pipeline_admin === 1,
              ...
              <div className="px-5 pb-4">
                ...
                <div className="flex flex-col gap-2">
                  ...
                  <label className="flex items-center gap-2 text-sm cursor-pointer select-none">
                    <input
                      type="checkbox"
                      checked={form.is_pipeline_admin}
                      onChange={e => setForm(p => ({ ...p, is_pipeline_admin: e.target.checked }))}
                      className="w-4 h-4 accent-slate-600"
                    />
                    <span>Pipeline 管理員 <span className="text-[11px] text-gray-500">（可在排程 Pipeline 設定 DB 寫入節點、管理可寫表白名單）</span></span>
                  </label>
                </div>
              </div>
