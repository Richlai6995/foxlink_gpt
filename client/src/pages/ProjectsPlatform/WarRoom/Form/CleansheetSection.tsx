...
export default function CleansheetSection({ project }: { project: ProjectDetail }) {
  ...
  const [lineCfg, setLineCfg] = useState<{ values: any[]; mults: any[] }>({ values: [], mults: [] })   // B-4' line×config 倍率
  ...
  async function loadAll(cf: number) {
    ...
    try {
      ...
      api.get<any>(token, `/bom/case/${cf}/line-config`).then((r) => setLineCfg({ values: r.values || [], mults: r.mults || [] })).catch(() => setLineCfg({ values: [], mults: [] }))
  ...
  function lineMultOf(lineCode: string, componentCode: string, valueId: number): number {
    const m = lineCfg.mults.find((x) => x.lineCode === lineCode && x.componentCode === componentCode && x.valueId === valueId)
    return m ? Number(m.multiplier) : 1
  }
  async function saveLineMult(lineCode: string, componentCode: string, valueId: number, value: string) {
    try {
      await api.put<any>(token, `/bom/case/${activeCf}/line-config`, {
        lineCode, componentCode, valueId, multiplier: value === '' ? 1 : Number(value),
      })
      setDirty(true)
      await loadAll(Number(activeCf))
      if (whatif?.active) dryRun()   // 沙盒中改倍率 → 自動試算
            ...
            <div className="space-y-1.5">
              ...
              <div className="mt-2 border-t border-dashed border-cortex-line pt-1.5">
                <div className="text-[10px] text-cortex-muted mb-1">
                  ⚙ Line × 配置 用量倍率 — 每條成本線對各「產品配置」的倍率(預設 1;<b>0 = 該配置不做此線</b>;yield 差異填如 0.05 / 1.7)。
                  subtotal = Σ(line 金額 × 倍率)→ OH/SG&A/Profit = subtotal × %,自動隨配置變 · 改完按「重算」生效
                </div>
                {lineCfg.values.length === 0 ? (
                  <div className="text-[10px] text-cortex-muted px-1">專案無變異軸 → 無配置可設倍率(先在 BOM 區建立變異軸)</div>
                ) : (
                  <div className="overflow-x-auto">
                    <table className="text-[10px] min-w-full">
                      <thead className="text-cortex-muted border-b border-cortex-line"><tr>
                        <th className="text-left px-1.5 py-0.5 sticky left-0 bg-white">Line</th>
                        <th className="text-right px-1.5 py-0.5">$/unit</th>
                        {lineCfg.values.map((v) => <th key={v.valueId} className="text-center px-1.5 py-0.5 whitespace-nowrap">{v.valueCode}</th>)}
                      </tr></thead>
                      <tbody>
                        {(detail?.simplifiedLines || []).map((r: any, i: number) => (
                          <tr key={`lc-${i}`} className="border-b border-cortex-line/30">
                            <td className="px-1.5 py-0.5 font-mono whitespace-nowrap sticky left-0 bg-white">
                              {r.line_code}
                            ...
                            {lineCfg.values.map((v) => {
                              ...
                              return (
                                <td key={v.valueId} className={`px-1.5 py-0.5 text-center ${m !== 1 ? 'bg-cortex-amber-bg/50' : ''}`}>
                                  <EditNum value={m} w="w-12" onSave={(val) => saveLineMult(r.line_code, r.component_code, v.valueId, val)} />
                                </td>
                              )
                            })}
                          </tr>
