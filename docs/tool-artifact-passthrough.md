...
>
> 草案日期:2026-04-24 · 實作日期:2026-04-26 · 狀態:**Phase 1 已 ship**
...
### Phase 1(本規劃書 scope)— 2026-04-26 已 ship

- [x] **DB schema migration** — [server/database-oracle.js](../server/database-oracle.js) `runMigrations()` 加 `MCP_SERVERS` / `SKILLS` 三欄(`passthrough_enabled` / `passthrough_max_bytes` / `passthrough_mime_whitelist`)+ 新建 `CHAT_ARTIFACTS` 表(IDENTITY PK + 13 欄 + 3 個索引,容錯 ORA-00955/01408)
- [x] **`toolResultPassthrough.js` 核心** — [server/services/toolResultPassthrough.js](../server/services/toolResultPassthrough.js) 單一入口 `detectPassthrough({ result, mimeHint, source, toolMeta })`;含 `sniffMime` / `extractSummary` / `renderSummaryForLLM`;`PASSTHROUGH_FEATURE_DISABLED=1` 全域 kill switch
- [x] **`skillHelpers.js`** — [server/services/skillHelpers.js](../server/services/skillHelpers.js) `toMarkdownArtifact` / `toHtmlArtifact`
- [x] **mcpClient 整合** — [server/services/mcpClient.js](../server/services/mcpClient.js) `callTool(... opts={})` 加 `returnRaw` 選項回 `{ text, parts, isError }`,只在 `server.passthrough_enabled=1` 時走新路徑;**其他 4 個 caller**(chartExecutor / pipelineRunner / workflowEngine / webex)零變動
- [x] **skillRunner 整合** — [server/routes/chat.js](../server/routes/chat.js) skill HTTP fetch 拿到 `data` 後對 `data.artifact` 做 opt-in 偵測(skill 不走 sniff,只看顯式回傳)
- [x] **chat.js SSE artifact event** — `tryPassthrough()` helper + `pendingArtifacts[]` 收集 + SSE `event: artifact`(streaming 中)+ INSERT `chat_messages` 後 batch INSERT `chat_artifacts` + SSE `event: artifact_persisted` 配對 client_id↔DB id
- [x] **chatArtifacts.js 路由** — [server/routes/chatArtifacts.js](../server/routes/chatArtifacts.js) `GET /api/chat/sessions/:sid/artifacts`(復載)+ `GET /api/chat/artifacts/:id`(單筆全文)+ `GET /api/chat/artifacts/:id/download`(attachment);全部 `WHERE user_id = req.user.id` 權限隔離
- [x] **Client `ArtifactCard`(inline MD + HTML iframe)** — [client/src/components/ArtifactCard.tsx](../client/src/components/ArtifactCard.tsx) MD 走獨立 ReactMarkdown(**不啟 rehype-raw**)+ HTML 走 `sandbox="allow-scripts"` iframe 限高 600px + `FullscreenModal`
- [x] **Client `copyMarkdown` + clipboard fallback** — 沿用既有 [client/src/lib/clipboard.ts](../client/src/lib/clipboard.ts)(`copyText` 已含 `execCommand` fallback for HTTP / 舊瀏覽器)
- [x] **Session 復載 artifacts** — [client/src/pages/ChatPage.tsx](../client/src/pages/ChatPage.tsx) `loadSession` 額外 GET artifacts、按 `message_id` group 後塞回 messages
- [x] **Admin UI(MCP + Skill passthrough 設定)**
  - MCP:[client/src/components/admin/McpServersPanel.tsx](../client/src/components/admin/McpServersPanel.tsx) 在 Inject/Answer radio 後**獨立 checkbox**(允許 inject + passthrough 共存),含 max bytes(KB)+ MD/HTML 雙 checkbox + 警語
  - Skill:[client/src/pages/SkillMarket.tsx](../client/src/pages/SkillMarket.tsx) 在「進階」分頁加同一 block + 加註「skill 必須回 artifact 欄位才生效」
  - 後端 INSERT/UPDATE 同步 — [server/routes/mcpServers.js](../server/routes/mcpServers.js) 主 SQL 直接帶 3 欄;[server/routes/skills.js](../server/routes/skills.js) 因主 SQL 太大,獨立 UPDATE statement(避免動既有 SQL)
- [x] **Audit log event** — `audit_logs.content` 加 `[passthrough] {source}:{tool} ({size}B, {mime})` prefix;**不**跑 `sensitive_keywords` 掃描(決策 D);fire-and-forget `.catch(() => {})`
- [x] **i18n 三語** — `chat.artifact.*` × 8 keys + `chat.passthrough.*` × 8 keys 同步加進 `zh-TW.json` / `en.json` / `vi.json`
- [x] **Help 系統新增「Passthrough 模式」說明** — [server/data/helpSeedData.js](../server/data/helpSeedData.js) 在 `u-skill` section 加 subsection(含「要 AI 分析請下載上傳」流程說明 + 敏感詞掃描跳過警告);bump `last_modified` → 2026-04-26
- [N/A] ~~metal_prices.js 升級示範~~ — **本次 scope 不含 metal_prices**。原計畫文件 §5.4 把它列為示範,但該 skill 屬另一獨立追蹤項目;passthrough 機制本身已具備泛用性,任何 skill 只要回 `{ artifact: {...} }` 即可。`skill_sources/` 下的範例 stub 與 manifest 屬實作附帶物,**不算 Phase 1 驗收項**
...
Phase 1 完成的定義(2026-04-26 ship 後實際狀態):

| # | 項目 | 狀態 |
|---|---|---|
| 1 | 既有 MCP servers / skills 預設行為**完全不變**(passthrough_enabled=0) | ✅ DB default 0、`returnRaw` opt-in、其他 4 個 callTool caller 零變動 |
| 2 | ~~metal_prices.js 升級後 token 省 >80%~~ | **不在本次 scope**(見 §12 N/A 註) |
| 3 | 找一個 MCP 工具回 HTML → iframe 正確 sandbox 渲染,scripts 不能存取 parent | 🟡 程式設計到位(`sandbox="allow-scripts"` 不給 `same-origin`),**待實機驗證** |
| 4 | Passthrough 內容 > 500KB → fallback 走 inject,不壞 flow | ✅ `detectPassthrough` size gate 命中時 return `{ passthrough:false, reason:'oversize:...' }`,unit smoke test 已過 |
| 5 | Session 切換回來 → artifact 仍顯示 | ✅ `loadSession` GET `/sessions/:id/artifacts` 已 wired |
| 6 | MD artifact `📋 複製 Markdown` 成功複製;HTML artifact `📥 下載` 產出 .html | ✅ MD 用 `copyText`(含 fallback),下載走 client-side Blob URL 不打 API |
| 7 | Admin UI 三語正常 | ✅ zh-TW / en / vi 全部加 16 keys |
| 8 | Audit log 有 passthrough event 紀錄 | ✅ `[passthrough] ...` prefix 寫入 `audit_logs`,event-only 不掃 sensitive_keywords |

**Smoke test(2026-04-26)**:7 個 unit case 全綠 — skill opt-in / MCP nested resource part / sniff fallback / disabled config / oversize / mime block / sniffMime helper。

**剩餘待辦**(實機驗證):
- 接一個會回 HTML 的 MCP 工具,確認 iframe 內 script 不能存 parent 的 cookie / localStorage
- 測超過 500KB 的 artifact fallback 行為
- 多 pod K8s 環境下 SSE artifact event 是否會被 nginx-ingress proxy buffer 切斷
...
| 項目 | 說明 |
|---|---|
| `chat_artifacts` INSERT 拿不到 `lastInsertRowid` | CLOB 欄位導致 `RETURNING id INTO` 觸發 ORA-22848,Oracle wrapper fallback 到 plain insert,結果 `lastInsertRowid: null`。`artifact_persisted` SSE event 因此不會 fire,但 client streaming 中已用 `client_id` 顯示完整 artifact、download 走 Blob URL 不需 DB id、session 復載拿新 list 帶 DB id — 不影響功能 |
| Streaming 中 artifact 不即時顯示 | 跟 `generated_files` / `charts` 一致,等 `done` 後一次 setMessages,UX 一致性優於即時性 |
| MarkdownRenderer 全站啟 `rehype-raw` | pre-existing 風險(不在本次 scope)。Artifact 本身用獨立 sanitized renderer 隔離。Inject 模式的 MD 仍經主 renderer,該風險屬另一個 scope |
| Helmet/CSP 全站未啟用 | 沒裝 helmet,iframe sandbox 不會被 CSP 擋。屬另一專案議題 |
...
| 2026-04-26 | **v1.0 ship** | **Phase 1 全項目實作完成**(metal_prices 範例除外,屬另案)。16 檔變動、501 行新增。修正 1 個 review-stage bug:`detectPassthrough` 對 MCP `resource` part 的 nested mimeType / text 偵測(原本只看 `p.mimeType`,漏了 `p.resource.mimeType`)。7 個 unit smoke case 全綠。實機驗證 + iframe sandbox / 500KB / nginx SSE buffer 測試列入後續 |
