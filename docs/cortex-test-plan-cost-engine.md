...
| 7 | 📦 BOM 區捲到底 →「🏭 多廠成本矩陣 → 前往獨立分頁」→ 右上「▶ 算缺格(N)」(全齊時無此鈕,用 Qty/PKG pills 切看) | 五個 config 欄位同上值 |
...
| 3 | 🧮 Cleansheet → 廠 tab 切 CN →「Compute」看 Material;切 VN →「Compute」 | **VN Material 比 CN 多（qty×主價）**;CN 不變(Cleansheet Compute = 強制重算,不吃矩陣快取) |
| 4 | chip 點 × 刪除 → VN 廠再 Compute | VN Material 還原 = CN |
...
| 6 | 🧮 Cleansheet:CN 廠 Compute → Material **2.70**;VN 廠 Compute → **2.97**(矩陣獨立分頁第一次「算缺格」亦同值) |
