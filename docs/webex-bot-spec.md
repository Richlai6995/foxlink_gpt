...
> 版本：1.4
> 日期：2026-03-29
> 狀態：已實作（Polling + Redis 分散鎖 + 稽核日誌 + 計費整合 + Typing Indicator + 檔案附件 + 新對話視覺分隔，4 Pod K8s 生產環境驗證通過）
...
| `client/src/components/admin/WebexLogsPanel.tsx` | 管理後台 Webex Bot 日誌頁籤（認證紀錄 + 對話稽核） |
...
| `server/database-oracle.js` | Migration：`CHAT_SESSIONS.SOURCE/WEBEX_ROOM_ID`、`USERS.WEBEX_BOT_ENABLED`、`AUDIT_LOGS.SOURCE`、新增 `WEBEX_AUTH_LOGS` 表 |
| `server/routes/users.js` | GET/POST/PUT 加入 `webex_bot_enabled` |
| `server/routes/admin.js` | `/audit-logs` 加 `source` 篩選；新增 `/webex-auth-logs` endpoint |
| `server/routes/chat.js` | `audit_logs` INSERT 加 `source='web'` |
| `server/services/tokenService.js` | 新增 `checkBudgetExceeded()` 共用函數 |
| `client/src/components/admin/UserManagement.tsx` | 加入「允許使用 Webex Bot」勾選框 |
| `client/src/components/admin/AuditLogs.tsx` | 加「來源」篩選器（全部/Webex/Web）+ 來源 badge |
| `client/src/pages/AdminDashboard.tsx` | 加入「Webex Bot 日誌」頁籤 |
| `client/src/types.ts` | `AuditLog` 介面加 `source` 欄位 |
| `server/.env` | 新增 Webex + Redis 相關環境變數 |
...
4. 指令分派
   ├─ '?'                          → 工具清單
   ├─ '/new' 及多種別名             → 新 session + webex_tmp 清理 + 視覺分隔線
   ├─ '/help'                       → 使用說明
   └─ 其他                          → AI Pipeline

5. 下載附件 → 暫存 uploads/webex_tmp/

6. 傳送 Typing Indicator（「⏳ 正在分析您的問題，請稍候...」）

7. 呼叫 AI（generateWithTools，非 SSE）

8. 刪除 Typing Indicator 訊息（deleteMessage）

9. processGenerateBlocks → 生成檔案

10. 截斷超長回應（> 4000 字元）

11. 儲存訊息、更新 session、記錄 token

12. sendMessage 文字回應 / sendFile 附件回傳
```
...
|---|---|
| `?` | 列出該使用者可使用的所有工具清單 |
| `/new`、`新對話`、`重置`、`new`、`/clear`、`/restart`、`/reset` | 強制開新對話 session（多種別名均有效） |
| `/help` | 顯示使用說明 |
...
```
━━━━━━━━━━━━━━━━━━━━━━━━
🔄 新對話開始（2026/03/29 14:30:00）
━━━━━━━━━━━━━━━━━━━━━━━━
請輸入您的問題。
```

時間戳為台北時區（Asia/Taipei），24 小時制。Webex 對話歷史仍保留（不刪除舊訊息），視覺分隔線讓用戶清楚辨識新舊對話邊界。

**觸發時同時執行：**
- 建立新 chat_session（DB）
- 清理 `uploads/webex_tmp/` 中超過 30 分鐘的殘留暫存檔
...
// 傳入 WEBEX_SYSTEM_SUFFIX 覆蓋 system 指示
...
【Webex 檔案生成規則】（此規則優先級最高）
6. 若需生成 Excel/Word/PDF/PPT/TXT，照常輸出完整 generate_xxx 代碼塊
7. 生成後說：「📎 檔案將以附件傳送，請稍候」
8. 絕對不要說「請點擊下載連結」、「請至 Web 介面下載」
9. Webex 模式下所有生成的檔案直接以附件附加，無需連結
`;
```
...
class WebexService {
  ...
  // 傳送純文字訊息，回傳 message id（供後續 deleteMessage 使用）
  async sendMessage(roomId, text, { markdown, parentId } = {}): Promise<string | null>
...
### 14.1 Migration 欄位（全部透過 safeAddColumn / createTable 自動執行）
...
-- 0 = 停用 Webex Bot，1 = 啟用（預設全體啟用）

-- audit_logs（區分對話來源）
ALTER TABLE AUDIT_LOGS ADD SOURCE VARCHAR2(20);
-- 'web' = Web UI，'webex' = Webex Bot，NULL = 舊資料（視為 web）

-- webex_auth_logs（新表：記錄每次 email 認證結果）
CREATE TABLE webex_auth_logs (
  id           NUMBER GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
  raw_email    VARCHAR2(200),   -- Webex 傳來的原始 email
  norm_email   VARCHAR2(200),   -- 正規化後（@foxlink.com.tw → @foxlink.com）
  user_id      NUMBER,          -- 比對到的 users.id，找不到時為 NULL
  user_name    VARCHAR2(200),
  username     VARCHAR2(200),
  status       VARCHAR2(30),    -- 'ok' | 'not_found' | 'disabled' | 'bot_disabled'
  room_type    VARCHAR2(20),    -- 'direct' | 'group'
  room_id      VARCHAR2(200),
  msg_text     VARCHAR2(200),   -- 訊息前 100 字預覽
...
| 情境 | 回應訊息 | auth_log status |
|---|---|---|
| 用戶 email 不在系統 | `⚠️ 您的帳號（{email}）未在系統中，請聯絡系統管理員。` | `not_found` |
| 用戶帳號停用 | `⚠️ 您的帳號目前已停用，請聯絡系統管理員。` | `disabled` |
| **webex_bot_enabled=0** | `⚠️ 您的帳號目前未開啟 Webex Bot 功能，如需使用請聯絡系統管理員。` | `bot_disabled` |
| 認證成功 | （正常處理，不回覆錯誤） | `ok` |
...
| 超過 daily/weekly/monthly 預算（block 模式） | `⚠️ 當日/本週/本月使用金額已達上限 $N（已使用 $X），請稍後再試。` | — |
| 超過預算（warn 模式） | 繼續執行，僅記錄 server log | — |
...
### 18.1 Console Log（Pod 標準輸出）

```
[WebexListener] Polling started (interval=8000ms, redis-lock=enabled)
[WebexListener] 3 active room(s)
[WebexListener] room="Alice Chen" → 2 msg(s)
[WebexListener] Dispatch: type=direct from="alice@foxlink.com" text="請問..."
[WebexListener] Skipped (lock held by another pod): msg=Y2lzY29zcGF...

[Webex] Incoming DM from="alice@foxlink.com" roomId="..."
[Webex][Auth] email lookup: raw="alice@foxlink.com" → normalized="alice@foxlink.com"
[Webex][Auth] ✅ user found: id=42 username="A12345" name="Alice" status=active
[Webex][Auth] User authenticated: id=42 username="A12345" role=user
[Webex] AI response: 342 chars, input=1280 output=256 tokens
[Webex][Auth] ❌ user NOT found for normalized email="bob@gmail.com"
[Webex][Budget] warn user=42 msg="本月使用金額已達上限..."
```

### 18.2 DB 持久化日誌（管理後台可查詢）

| 資料表 | 寫入時機 | 說明 |
|---|---|---|
| `webex_auth_logs` | 每次訊息到達、email 解析後 | 記錄原始 email、正規化 email、對應用戶、認證結果 |
| `audit_logs`（source='webex'） | AI 回答完成後 | 記錄對話內容、敏感詞偵測結果 |
| `token_usage` | AI 呼叫完成後 | 記錄 input/output tokens、費用（與 Web UI 合計計費） |

管理後台查詢路徑：
- **認證紀錄**：後台 → Webex Bot 日誌 → 認證紀錄
- **對話稽核**：後台 → Webex Bot 日誌 → 對話稽核（或 稽核日誌 → 來源篩選 Webex）
- **Token 費用**：後台 → Token 與費用統計（Webex 與 Web UI 合計顯示）
...
| 超過預算 DM | `⚠️ 當日使用金額已達上限...` 並中止，不呼叫 AI |
| 後台查認證紀錄 | webex_auth_logs 顯示 ok/not_found/disabled/bot_disabled |
| 後台稽核日誌篩選 Webex | 顯示 source=webex 的對話記錄 |
| 4 Pod 同時運行 | 每則訊息只有 1 Pod 處理（Redis lock 驗證） |
| AI 處理期間 | 顯示「⏳ 正在分析您的問題，請稍候...」，回應後自動刪除 |
| 傳送 `新對話` / `/clear` | 等同 `/new`，顯示時間戳視覺分隔線 |
| 要求生成 Excel | 回傳 xlsx 附件（不含下載連結文字） |
...
### 24.1 Typing Indicator

AI 呼叫前先傳送一則「處理中」訊息，AI 回應完成後自動刪除，讓用戶知道 Bot 正在處理：
...
**實作方式：** `sendMessage()` 回傳 message id → AI 完成後呼叫 `DELETE /v1/messages/{id}`

> **注意：** Webex API 只允許 Bot 刪除自己發的訊息（Bot token 建立的訊息），用戶訊息無法刪除。

### 24.2 AI 生成檔案以附件直接回傳（不含下載連結）

`WEBEX_SYSTEM_SUFFIX` 中加入檔案生成規則，確保 AI 不輸出「請至 Web 介面下載」等文字：

- AI 仍正常輸出 `` ```generate_xlsx:filename.xlsx``` `` 代碼塊
- `processGenerateBlocks()` 解析並生成實體檔案
- 生成後呼叫 `webexService.sendFile()` 以附件回傳用戶
- AI 說明文字改為「📎 檔案將以附件傳送，請稍候」
...
```
/new, /重置, 新對話, 重置, new, /clear, /restart, /reset
```
...
**設計考量：** Webex API 只允許 Bot 刪除自己的訊息，無法刪除用戶訊息，因此採視覺分隔取代刪除舊訊息。對話歷史仍完整保留在 Webex 介面與 FOXLINK GPT 對話歷史。

### 24.4 暫存檔定期清理

**觸發 `/new` 時：** 立即清理 `uploads/webex_tmp/` 中超過 30 分鐘的殘留檔案。

**Server 啟動後每小時（`setInterval`）自動清理：**

| 目錄 | TTL | 說明 |
|---|---|---|
| `uploads/generated/` | 24 小時 | AI 生成的 xlsx/pdf/docx/pptx 暫存 |
| `uploads/webex_tmp/` | 24 小時 | Webex 附件下載暫存 |

伺服器啟動後立即執行一次，之後每小時執行，log：`[TmpCleanup] {dir}: removed N stale files`
...
## 23. 稽核日誌與計費整合（v1.3 新增）

### 23.1 對話稽核

所有 Webex 對話與 Web UI 對話統一記錄在 `audit_logs` 表，以 `source` 欄位區分：

| source 值 | 來源 |
|---|---|
| `'web'` | Web UI 對話 |
| `'webex'` | Webex Bot 對話 |
| `NULL` | v1.3 以前的舊資料（視為 web） |

**管理後台查詢：**
- **稽核日誌**頁籤：加入「來源」篩選器，可選「全部 / Webex Bot / Web UI」，每筆顯示 Webex（綠）/ Web（藍）badge
- **Webex Bot 日誌 → 對話稽核**：只顯示 source=webex，含敏感詞偵測

### 23.2 Email 認證紀錄（webex_auth_logs）

每則訊息到達時，email 解析結果會寫入 `webex_auth_logs`：

| 欄位 | 說明 |
|---|---|
| `raw_email` | Webex 傳來的原始 email |
| `norm_email` | 正規化後（@foxlink.com.tw → @foxlink.com） |
| `user_id` | 比對到的 user，找不到為 NULL |
| `status` | `ok` / `not_found` / `disabled` / `bot_disabled` |
| `room_type` | `direct`（DM）/ `group`（群組） |
| `msg_text` | 訊息前 100 字預覽 |

**管理後台查詢：** 後台 → Webex Bot 日誌 → 認證紀錄
可篩選結果（✅ 成功 / ❌ 不存在 / ⛔ 停用 / 🔕 Bot 停用），可匯出 CSV。

### 23.3 Token 計費

Webex Bot 的 token 使用與 Web UI **完全整合到同一 `token_usage` 表**：
- 計費規則相同（daily/weekly/monthly budget 上限）
- `checkBudgetExceeded()` 共用函數，AI 呼叫前先檢查，超額則拒絕並回覆提示
- admin 角色豁免預算檢查
- Token 統計：後台 → Token 與費用統計（Webex + Web UI 合計）

### 23.4 Admin API

| Endpoint | 說明 |
|---|---|
| `GET /api/admin/audit-logs?source=webex` | 只取 Webex 對話稽核 |
| `GET /api/admin/audit-logs?source=web` | 只取 Web UI 對話稽核 |
| `GET /api/admin/webex-auth-logs` | 取 email 認證紀錄（支援日期/status 篩選） |
