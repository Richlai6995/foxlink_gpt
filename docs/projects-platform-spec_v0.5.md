...
> **狀態**:v0.5 規劃稿(取代 [quote-system-spec.md](./quote-system-spec.md) v0.3.5 的核心定位)
> **日期**:2026-05-26 最後更新(v0.5 補真實案校驗 — Variant / NRE Section / Packaging Sub-form / Multi-Factory Matrix)
> **作者**:資訊部
...
- ✅ **Variant Dimension(§11.3.5)— 2026-05-26 v0.5 補**:同一專案內單軸 ≤ 5 variant(CMF 黑/白、L/R 手版),variant-aware fields,UI tab switcher + roll-up widget
- ✅ **NRE Costs Section(§11.3.6)— 2026-05-26 v0.5 補**:QUOTE_STANDARD 預設 11 項 NRE 欄位 + 自訂 child-table,可選機密
- ✅ **Packaging Sub-form(§11.3.7)— 2026-05-26 v0.5 補**:`packaging_bom` child-table 8 欄(spec/qty/price/vendor/LT/note),3 種預設範本(Mouse / Headset / Connector)+ pallet compliance enum
- ✅ **Multi-Factory Cost Matrix(§11.3.8)— 2026-05-26 v0.5 補**:**修正** 2026-04-29 決議——三廠對比**資料**屬 Phase 1 必要(客戶交付物),**AI 解讀**仍留 Phase 2+(原 #14 移除點修正)
...
### 11.3.5 ✅ Variant Dimension(CMF / 規格軸)— 2026-05-26 v0.5 補

> **背景**:真實 RFQ 常見「同一專案內多 SKU」場景(如 SteelSeries Rival 3 黑/白雙色、Logitech 滑鼠左/右手版),共用 EE BOM 但 ME BOM / 部分 PKG 分開。原本若強迫每 variant 開獨立專案會造成成員 / 對話 / NRE 重複,治理變難。

#### 11.3.5.1 概念

Form template 可在 `qp_form_template_calculations` 或新表 `qp_form_template_variants` 宣告 **variant 軸**(目前 Phase 1 限 **單軸 ≤ 5 variant**,避免笛卡兒爆炸):

```sql
CREATE TABLE qp_form_template_variants (
  id, template_id,
  axis_key,                           -- 'cmf_color' | 'left_right_hand' | 'region_sku'
  axis_label_i18n,
  variant_values,                     -- JSON array: ['black','white'] | ['CN','US','EU']
  cardinality                         -- 必須等於 variant_values 長度
);
```

#### 11.3.5.2 Field 行為

每個 field 可標 **variant-aware**:

```sql
ALTER TABLE qp_form_template_fields ADD COLUMN
  variant_scope ENUM('shared','per_variant') DEFAULT 'shared';
```

- `shared`:跨 variant 同值(如:客戶名稱、EE BOM、PKG)
- `per_variant`:每 variant 一個值(如:ME BOM、material_cost、total_qty)

#### 11.3.5.3 Form instance 儲存

```sql
ALTER TABLE qp_form_field_values ADD COLUMN
  variant_key VARCHAR(64) NULL;       -- shared = NULL · per_variant = 'black' / 'white'

-- UNIQUE 改為:(instance_id, field_key, variant_key)
```

#### 11.3.5.4 UI

- Form 頂部新增 **Variant Tab Switcher**(若 template 有 variants):`[ 全部 | Black | White ]`
- `shared` field 在所有 tab 都顯示相同(編輯任一處同步)
- `per_variant` field 在「全部」tab 顯示為 mini-table(N 欄),在個別 variant tab 只顯示該 variant 的值
- 成本核算 / NRE / Cleansheet 自動聚合到「Variant Roll-up」widget

#### 11.3.5.5 Quantity 分配

- `project.qty` = 年總量(417K)
- `variant.qty_share` = 比例(Black 80% / White 20%)→ 每 variant 實際 qty 推算
- 也可選擇直接給絕對數(qty_absolute 模式)

#### 11.3.5.6 Phase

- **Phase 1**:單軸 ≤ 5 variant,UI tab switcher,roll-up widget
- **Phase 2**:雙軸交叉(CMF × Region),笛卡兒積可達 9~25 cell,改用矩陣 UI
- **Phase 3**:variant-conditional fields(如 White CMF 才需要 paint cost 欄位)

---

### 11.3.6 ✅ NRE Costs Section(QUOTE_STANDARD 預設)— 2026-05-26 v0.5 補

> **背景**:NRE(Non-Recurring Engineering)成本長期散在 RACI 表中(§18.1.5),沒有當成 form 的獨立 section,實作時 PM 容易漏項。對齊真實案例(Foxlink 多家 RFQ),NRE 11 項固定欄位足以涵蓋 90% gaming peripheral / connector RFQ 場景。

#### 11.3.6.1 預設欄位(11 項 + 自訂)

| field_key | label | data_type | R(Responsible) | A(Accountable) | Optional |
|---|---|---|---|---|---|
| `nre_build_cost` | Build Cost(試產費) | currency | SMT/NPI team | EPM | No |
| `nre_emc_test` | EMC Test + Cert | currency | QA team | DPM | Yes(客戶自付則 0) |
| `nre_emc_debug` | EMC Debugging 預算(hrs) | currency | RD QA | DPM | Yes |
| `nre_compatibility` | Compatibility Test | currency | QA team | DPM | Yes |
| `nre_dve` | DVE NRE(Design Verification Eng) | currency | RD | DPM | No |
| `nre_travel` | Travel Expense | currency | DPM | DPM | Yes |
| `nre_dev_npi_labor` | Dev. + NPI Labor Cost | currency | DPM team | DPM | No |
| `nre_reliability` | Reliability Test(含 EV/DV) | currency | QA team | DPM | No |
| `nre_pkg_ret` | Package RET(Reliability) | currency | PKG team | MPM | No |
| `nre_ort` | ORT(Ongoing Reliability Test) | currency | QA team | DPM | No |
| `nre_mte_fixture` | Unique Fixtures(MTE NPI) | currency | NPI EPM | DPM | No |
| `nre_tooling` | Tooling(模具改費) | currency | 廠商 / 塑件PM | DPM | Yes |

加上 `nre_other_items[]`(child-table,自訂項目)。

#### 11.3.6.2 計算 + 顯示

- `nre_total = SUM(*) over enabled items`
- UI 表格顯示:Item / U/P / Qty / Sub-Total / Delta(原值 → 議價後) / Remark
- 雙欄並列「Original Quote」vs「Updated Quote」(對應真實 NRE Summary 樣式)
- `nre_total / project.qty` 攤提到單價(顯示為附註,非加進 unit cost)

#### 11.3.6.3 機密

- 預設 `is_confidential_default = true`(對 PARTICIPANT 走 RANGE display)
- HOST / DPM / BPM 看明文

---

### 11.3.7 ✅ Packaging Sub-form(child-table)— 2026-05-26 v0.5 補

> **背景**:真實 RFQ packaging 常見 10-20 項細料(Gift Box / Inner Pad / Seal Sticker / Labels / Pallet / UN 認證 label …),每項有獨立 spec / vendor / 單價。原方案是當成單一 BOM cost 欄位,看不到結構,治理弱。

#### 11.3.7.1 Field 設計

QUOTE_STANDARD template 新增 `packaging_bom` field,`data_type = child_table`,綁定 schema:

```yaml
field_key: packaging_bom
data_type: child_table
schema:
  - col_key: pkg_no             # 流水號
    label: NO.
    type: number
  ...
  - col_key: unit_price
    ...
    type: currency
    is_confidential_default: true
  - col_key: vendor
    label: Vendor / Source
    type: text
  - col_key: lead_time_wk
    label: LT (週)
    type: number
  - col_key: note
    label: 備註 / Dieline ref
    type: text
calculations:
  - calc_key: pkg_total_per_unit
    formula: SUM(qty * unit_price)
  - calc_key: pkg_vendor_count
    formula: DISTINCT(vendor).count
```

#### 11.3.7.2 預設範本(可一鍵載入)

提供常見 packaging structure 範本(Phase 1 三個):

- **Mouse / Keyboard standard**:Gift Box, Inner Pad, Inner Partition, Seal Sticker, PE Bag, PIG, PID Label, Box Label, Master Carton, Master Carton Label, SN Label, Logo Tape, Pallet Material, UN3481 Label(對齊真實案 14-16 項)
- **Headset / Audio**:加 Foam Insert, Cable Tie
- **Connector / Cable**:Anti-static Bag, Reel, Label only

#### 11.3.7.3 跟主 form 關係

- `packaging_bom` 是主 form 內欄位,**不**獨立成 child project
- 各列 vendor 走「詢價彙總」section 同一 vendor master(可 cross-reference)
- 變更 propagate 到 cost section 的 `material_cost_pkg`

#### 11.3.7.4 Pallet 規範外掛

加 `pallet_compliance` field(enum):`EU EPAL` / `US GMA` / `APAC Plywood`(對齊客戶 Pallet Requirement V20)— 影響運費 / 重量計算,但細節不入 form。

---

### 11.3.8 ✅ Multi-Factory Cost Matrix(三廠對比資料,AI 解讀仍待 Phase 2+)— 2026-05-26 v0.5 補

> **背景**:對齊 2026-04-29 決議的修正版本。原 spec(2026-04-28)把整個 #14「三廠對比」移到未來規劃,但真實 RFQ(SteelSeries / 多客戶)幾乎都會要求 CN/VN/TW 三廠對比資料。**重新評估**:**矩陣資料本身**屬 Phase 1 必要(客戶交付物);**AI 解讀**仍留 Phase 2+。

#### 11.3.8.1 範圍邊界

| 元件 | Phase 1 | Phase 2 | Phase 3+ |
|---|---|---|---|
| 三廠 BOM cost 矩陣(資料) | ✅ | | |
| 三廠 Cleansheet 矩陣 | ✅ | | |
| 三廠 NRE 矩陣(若各廠 fixture / tooling 不同) | ✅ | | |
| 三廠 × 多 PKG option 二維矩陣 | ✅(table UI) | | |
| **AI 解讀**(「為何 VN 比 CN 貴 $0.5?」自然語言 explain) | ❌ | ✅(`#14 deepening`) | |
| **AI 推薦**(「客戶要求 VN,但 TW 同 SLA 便宜 8%,要 push 建議嗎?」) | ❌ | | ✅ |
...
```yaml
field_key: factory_matrix
data_type: matrix
schema:
  axes:
    - axis_key: factory
      label: 製造廠
      values: ['CN', 'VN', 'TW']    # admin 可加 'IN' 'MX' 等
    - axis_key: pkg_option
      label: 包裝方案
      values: ['A', 'B', 'C']        # 對應 packaging_bom 內的版本
  cells:
    - cell_key: material_cost
      data_type: currency
      is_confidential_default: true
    - cell_key: prime_cost          # material + labor
      data_type: currency
    - cell_key: total_cost_exfactory # + SG&A + profit
      data_type: currency
calculations:
  - cheapest_factory_option:
      formula: ARGMIN(total_cost_exfactory)
  - factory_spread:
      formula: MAX(total_cost) - MIN(total_cost)
```

#### 11.3.8.3 UI

- **9-cell 表格**(3 廠 × 3 option),色彩深淺對應價差
- HOST 視角:明文 + 自動標亮最低 cell
- PARTICIPANT(非機密成員):走 RANGE display(`$10~$12`)
- 點 cell → drill 到該 (廠, option) 的 BOM cost + Cleansheet 明細

#### 11.3.8.4 與 §16 跨專案儀表板的關係

跨專案儀表板可加 widget「各廠歷史平均 spread」,但解讀 / 推薦仍 Phase 2+。

#### 11.3.8.5 客戶指定廠

若客戶 PRD 指定 specific factory(常見場景),系統允許 PM 在 `factory_matrix` 標記 `mandatory_factory_id`(系統不阻止對比其他廠,但 final quote 只用客戶指定那欄)。
