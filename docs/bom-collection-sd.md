...
## v0.2 → v0.3 改動摘要(2026-05-30)

| # | 章節 | 改動 |
|---|---|---|
| 1 | §16(新)| **Multi-Factory Cost Matrix DB schema 升級**:從 `project.data_payload.factory_matrix` JSON CLOB 拆成 4 張關聯表(`project_factory_matrix` / `pfm_factory` / `pfm_pkg_option` / `pfm_cell`)|
| 2 | §16.4 | BOM lock 觸發 propagate 到 `pfm_cell.material_cost` 的 SQL flow |
| 3 | §16.5 | MVA 架構標 ⏳ 待確認(6 項 TBD)· 跟 BOM 開工同步 |
| 4 | §15 | TBD 清單擴充 11 → 15 項(加 4 個 factory_matrix 相關) |
| 5 | 附錄 A | Claude Code P0-P3 加 factory_matrix migration + propagate hook(P3 末)|
...
## 14. 與 spec v0.5 主規格的對應(將於 §16 完成後完整對齊)
...
12. **MVA source**:從 cleansheet upload / ERP cost rollup / manual 哪個?(§16.5 待確認)
13. **MVA components 細度**:Labor / Equipment / Overhead 拆 3 欄,還是只存單一 `mva_per_unit`?(§16.5 待確認)
14. **MVA per variant 差異**:Black/White 的 MVA 真的不同嗎?(色料噴塗工時可能差)還是全 variant 共用?(§16.5 待確認)
15. **MVA per pkg option 差異**:不同包裝方案組裝工時是否影響 MVA?(§16.5 待確認)

---

## 16. ⭐ Multi-Factory Cost Matrix DB schema 升級(v0.3 新增 · 對齊 spec v0.5 §11.3.8)

> **完整內容**:請見獨立文件 [`factory-matrix-schema-sd.md`](./factory-matrix-schema-sd.md)
> **本章為摘要**,給 Claude Code 跟 BOM 主流程一起開工時用。

### 16.1 為何要升級

Phase 1-3 ship 的 `project.data_payload.factory_matrix` 是 JSON CLOB · 9 個結構性問題:

| 維度 | JSON CLOB 現況 | 4 表升級後 |
|---|---|---|
| Version chain | ❌ 無 | ✅ `version_no` UNIQUE |
| BOM 連動 | ❌ ad-hoc | ✅ `bom_instance_id` FK |
| Audit cell-level | ❌ JSON 改了不知道 | ✅ per cell `material_cost_updated_at` |
| 跨 project 統計 | ❌ parse JSON | ✅ SQL aggregate |
| Variant 整合 | 結構固定 | ✅ `variant_key` 動態 |
| 廠別擴充 | 改 axes 重寫 | ✅ INSERT row |
| Lock 狀態 | ❌ 無 | ✅ `state=LOCKED` + audit |
| 機密 mask | 整 JSON 粗暴 | ✅ per cell 細緻 |
| Phase 2 form engine | 要重搬 | ✅ 直接對應 `data_type=matrix` |

### 16.2 4 張表

```
project_factory_matrix       ← 主表 · 一 project × version 一張
  ├ pfm_factory             ← 廠別軸 · 3 rows CN/VN/TW · 含 MVA + erp_org_id
  ├ pfm_pkg_option          ← 包裝軸 · 3 rows A/B/C · 含 pkg_cost
  └ pfm_cell                ← cell · 3×3×N variant = 9N rows · 含 material/prime/total
```

詳細 Oracle 23 AI schema 見 [`factory-matrix-schema-sd.md`](./factory-matrix-schema-sd.md) §2。

### 16.3 SteelSeries 案拆解(seed 對應)

當前 `project.data_payload.factory_matrix` JSON 拆成:

| 表 | rows | 內容範例 |
|---|---|---|
| `project_factory_matrix` | 1 | project_id=61 / sga_profit=0.75 / suggested=11.87 / annual_qty=418000 / annual_rev=4.96M / recommended=CN-A |
| `pfm_factory` | 3 | CN(mva=1.86, erp_org=83)/ VN(mva=1.43, erp_org=2463)/ TW(mva=3.00, erp_org=...) |
| `pfm_pkg_option` | 3 | A 標準 $1.275 / B 減塑 $1.175 / C FSC premium $2.66 |
| `pfm_cell` | **18** | 3 廠 × 3 PKG × 2 variant(black/white)= 18 cells |

### 16.4 BOM lock 後 propagate flow

```
[DPM lock bom_instance]
     ↓
[trigger propagateToFactoryMatrix(bom_instance_id)]
     ↓
[per (variant_key, factory_id, pkg_option_id) 跑 SUM 並 update pfm_cell]
     ↓
[重算 prime_cost = material + factory.mva]
     ↓
[重算 total_cost_exfactory = prime + matrix.sga_profit + pkg.cost]
     ↓
[標記 is_min_in_variant per variant]
     ↓
[寫 audit_log: factory_matrix_propagated · payload = { from / to / delta }]
```

詳細 SQL 見 [`factory-matrix-schema-sd.md`](./factory-matrix-schema-sd.md) §3。

### 16.5 MVA 架構 ⏳ 待確認(跟 BOM 開工同步)

業主表示 **「等 MVA 架構確認一起開工」**。MVA 6 個待確認問題:

| # | 問題 | 影響 |
|---|---|---|
| MVA-1 | **MVA source**:cleansheet upload / ERP cost rollup / manual 哪個? | `pfm_factory.mva_source` 欄位設計 |
| MVA-2 | **MVA components 細度**:Labor / Equipment / Overhead 拆 3 欄?還是單一 `mva_per_unit`? | `pfm_factory` 加 3 子欄或維持 1 欄 |
| MVA-3 | **MVA per variant** 差異(色噴塗工時 black/white 不同)?| `pfm_factory_mva_variant` 新增表 vs 共用 |
| MVA-4 | **MVA per pkg option** 差異(包裝組裝工時)?| 同上 |
| MVA-5 | **MVA version chain**:跟 BOM 同步 lock 嗎? | 是否加 `pfm_mva_version` |
| MVA-6 | **MVA 來源 ERP SQL**:若走 ERP rollup,SQL 怎麼寫? | 工時影響 1-2 週 |

業主拍板前,本 SD 跟 factory-matrix-schema-sd 都標 ⏳。

### 16.6 Phase 規劃

| Phase | 工時 | 內容 |
|---|---|---|
| Phase 1(已 ship)| — | JSON CLOB 留著,UI 走 `FactoryMatrixSection.tsx` 從 `data_payload.factory_matrix` 讀 |
| **Phase 1 末 + Phase 2 初**(本章範圍)| 4-5 天 | migration 010-新版 + service + propagate hook + UI 改寫從 4 表讀 + backward compat 期 |

### 16.7 API endpoints(摘要)

| Method | Endpoint | 說明 |
|---|---|---|
| GET | `/api/projects/{pid}/factory-matrix` | 取最新 matrix(自動 fallback `data_payload.factory_matrix` JSON)|
| POST | `/api/projects/{pid}/factory-matrix` | 建新 version |
| PATCH | `/api/projects/{pid}/factory-matrix/{mid}/cells/{cid}` | 改單 cell |
| POST | `/api/projects/{pid}/factory-matrix/{mid}/lock` | DPM lock |
| POST | `/api/projects/{pid}/factory-matrix/{mid}/propagate-from-bom` | 手動觸發 BOM propagate(自動觸發見 §16.4) |
| GET | `/api/projects/{pid}/factory-matrix/{mid}/audit` | cell-level audit log |

完整 endpoint 設計見 [`factory-matrix-schema-sd.md`](./factory-matrix-schema-sd.md) §7。

### 16.8 UI 整合

`FactoryMatrixSection.tsx` 改寫:
- 棄用直接讀 `project.data_payload.factory_matrix`
- 改打 `GET /api/projects/{pid}/factory-matrix` 拉 4 表 join 結果
- backward compat:API 自動偵測 — 若無 `project_factory_matrix` row 就 fallback 回 `data_payload.factory_matrix` JSON(Phase 1 demo 不破)
- DPM lock BOM 後 UI 自動 reload cells(socket push)

---

## 附錄 A · 給 Claude Code 的實作優先順序(v0.3)

> ⏳ **前置條件**:`§16.5` MVA 6 項待確認(MVA-1 ~ MVA-6)業主拍板後才開工 P0。

```
P0 (週 1-3):
  - Schema 建表(本 SD §2 BOM 全 10 張 + §16 / factory-matrix-sd §2 factory_matrix 4 張)
  - 資料政策 §3.0 包裝層(multiOrgServiceAdapter)
  - ERP SQL §3.2.1 / §3.2.2 / §3.2.3
  - 基本 CRUD API §5.1 / §5.2 + §16.7 factory_matrix CRUD
...
P1 (週 4-6):
  ...
  - factory-matrix-sd §3 從 ERP 同步 erp_org_id → pfm_factory
...
P2 (週 7-9):
  ...
  - factory-matrix-sd §3 propagate service(BOM lock → pfm_cell.material_cost UPDATE)

P3 (週 10-12):
  - 機密策略 §6.2.4 + factory-matrix-sd §5 per-cell mask
  - Audit log §9.5 完整覆蓋 + factory-matrix-sd §6 pfm_cell_audit
  - BOMSection.tsx 整合進 FormPanel §11
  - FactoryMatrixSection.tsx 改寫從 4 表讀(backward compat)· factory-matrix-sd §4
  - Variant case 整合 §12(SteelSeries 案 EE 共用 / ME 分版實測)
  - MVA per-variant / per-pkg 差異支援(若 MVA-3 / MVA-4 業主確認需要)
  - 整合測試 + UAT pilot
```
