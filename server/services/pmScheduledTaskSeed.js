...
═══ 當日 11 金屬實際報價(forecast 必須以此為 anchor)═══
{{pm_current_prices}}
...
⚠️ **forecast anchor 紀律(MUST follow)**:
- predicted_mean **必須以「當日 11 金屬實際報價」段裡的 price 為 base**,不准用訓練資料記憶的 2024 年金屬價推
- 7 天 horizon 通常落在 current_price ± 3-8%(一般)/ ±15%(極端事件)
- 超出 ±20% 幾乎一定錯,請重新校準後再輸出
- 若 current_price 缺(段裡寫「無資料」),該金屬 predicted_* 全部填 null + confidence='low'`;
  ...
  return {
    ...
    email_body: '今日金屬市場日報如下(附件 docx 為可編輯版本):\n\n{{ai_response}}',
    status: 'paused',
    pipeline_json: JSON.stringify(pipeline),
  };
}
...
2. **11 金屬週漲跌幅統計 — 必須輸出完整 markdown 表格(這是採購最看重的欄位,不能省)**

   表格上方先寫(從漲跌幅計算,不影響表格排序):
     - **領漲板塊**:前 3 名(漲幅最大的 3 個金屬)
   ...
   | :---: | :--- | ---: | ---: | :---: | :--- |
   | 🟢 | **銅 (Cu)** | 13,819 (6/1) | 13,661 (6/8) | **-1.14%** | 美元高息壓抑,但 AI 基建需求與美國潛在關稅引發提前備貨,下檔具強勁支撐。 |
   | 🟢 | **鋁 (Al)** | 3,735 (6/1) | 3,669.5 (6/8) | **-1.75%** | 受美元強勢拖累,然供應鏈轉向第三國與半導體設備擴產維持實體剛性需求。 |
   | 🟢 | **鎳 (Ni)** | 18,800 (6/1) | 18,340 (6/8) | **-2.45%** | 終端買盤觀望導致區間拉回,印尼供給調控政策持續為價格長線築底。 |
   | 🟢 | **鋅 (Zn)** | 3,517 (6/1) | 3,519 (6/8) | **+0.06%** | 區間震盪,鋼鐵終端需求平穩,煉廠加工費 (TC) 持平。 |
   | 🟢 | **鉛 (Pb)** | 2,005 (6/1) | 1,990 (6/8) | **-0.75%** | 精礦供應緊繃及部分電池替換需求發揮支撐力道,跌幅相對收斂。 |
   | 🟢 | **錫 (Sn)** | 56,100 (6/1) | 51,850 (6/8) | **-7.58%** | 印尼出口暫停疑慮緩和,投機部位獲利了結,跌幅居首。 |
   | 🟢 | **金 (Au)** | 4,403 (6/1) | 4,352 (6/8) | **-1.16%** | 美元反彈、降息預期延後,金價月線收斂。 |
   | 🟢 | **銀 (Ag)** | 76.27 (6/1) | 68.60 (6/8) | **-10.06%** | 工業需求疲弱 + 美元壓抑,白銀大跌。 |
   | 🟢 | **鉑 (Pt)** | 1,820 (6/1) | 1,784 (6/8) | **-1.98%** | PGM 板塊整體疲弱,氫能應用尚未大規模放量。 |
   | 🟢 | **鈀 (Pd)** | 1,360 (6/1) | 1,261 (6/8) | **-7.28%** | 燃油車滲透率下滑,觸媒需求結構性走弱。 |
   | 🟢 | **銠 (Rh)** | 8,300 (6/1) | 8,000 (6/8) | **-3.61%** | 電動車快速取代燃油車,貴金屬避險買盤退場。 |
   \`\`\`

   **欄位定義**(每欄都必須填,不可省略):
     - 燈號 — 🔴(漲)/ 🟢(跌)/ 🟡(持平,|漲跌| < 0.5%)的 emoji
     - 金屬品種 — \`**中文名 (英文代碼)**\` 例:\`**銅 (Cu)**\`
     - **上週報價** — 取 \`{{pm_weekly_changes}}\` anchor 區塊每行「一週前」那個價(箭頭左側,數字千分位逗號)+ **括號標日期** \`13,819 (6/1)\`(該 baseline 的真實資料日期)
     - **本週最新報價** — 取 \`{{pm_weekly_changes}}\` anchor 區塊每行「最新」那個價(箭頭右側)+ **括號標日期** \`13,661 (6/8)\`
...
═══ 燈號規則(只用 emoji 圖示,絕對不可用 HTML / CSS)═══
**輸出時表格的「燈號」「狀態」欄位必須是以下 emoji 之一**(直接複製貼上):
- 上漲(正漲幅、強勢、領漲)→ 🔴
- 下跌(負漲幅、偏弱、領跌)→ 🟢
- 持平(漲跌幅 < ±0.5%)→ 🟡

**禁止事項(絕對不可出現):**
1. ❌ 不可用 \`<span style="color:red">\` / \`<span style="color:green">\` 等 HTML/CSS 包裹文字
...
**重要**:full_content 必須是「**完整可獨立閱讀的 markdown 全文**」,**不可以**用「詳見上方 markdown 報告全文」或「請參考上文」等簡寫偷懶。落地的這份 JSON 之後會獨立顯示在 PM 平台的「週報草稿」頁,user 看不到 prompt 上方的 markdown,只看 full_content。所以必須把完整的 5 個段落(市場主軸 / 11 金屬統計 / 重大事件 / 下週展望 / 採購策略)全部塞進 full_content,跟上方 markdown 內容一致(可重複,寧可冗長也別省略)。
...
2. **11 金屬月漲跌統計 — 必須輸出完整 markdown 表格(這是採購最看重的欄位,不能省)**

   表格上方先寫(從月漲跌幅計算,不影響表格排序):
     - **Top 3 漲幅**(月漲幅最大的 3 個)
     - **Top 3 跌幅**(月跌幅最大的 3 個)
   ...
   | :---: | :--- | ---: | ---: | :---: | :--- |
   | 🔴 | **銅 (Cu)** | 13,150 (5/9) | 13,819 (6/1) | **+5.1%** | 美元持續走弱、AI 電網建設驅動長線需求,月內庫存連降。 |
   | 🟢 | **鋁 (Al)** | 3,883 (5/9) | 3,795 (6/1) | **-2.3%** | 中國電解鋁供應回升,終端需求復甦不如預期。 |
   | 🔴 | **鎳 (Ni)** | 18,712 (5/9) | 19,050 (6/1) | **+1.8%** | 印尼配額審查趨嚴,不鏽鋼採購回穩。 |
   | 🟢 | **鋅 (Zn)** | 3,605 (5/9) | 3,555 (6/1) | **-1.4%** | 鍍鋅板需求疲軟,煉廠加工費下調。 |
   | 🟢 | **鉛 (Pb)** | 2,074 (5/9) | 2,008 (6/1) | **-3.2%** | 鉛酸電池替換需求疲弱,庫存累積壓抑。 |
   | 🔴 | **錫 (Sn)** | 52,810 (5/9) | 56,400 (6/1) | **+6.8%** | 印尼出口受限延續,半導體封測需求穩定。 |
   | 🟢 | **金 (Au)** | 4,657 (5/9) | 4,555 (6/8) | **-2.2%** | 美元反彈、降息預期延後,金價月線收斂。 |
   | 🔴 | **銀 (Ag)** | 72.97 (5/9) | 75.66 (6/1) | **+3.7%** | 光電產業裝置量續創新高,工業需求支撐。 |
   | 🟢 | **鉑 (Pt)** | 2,000 (5/9) | 1,993 (6/8) | **-0.35%** | PGM 板塊整體疲弱,氫能應用尚未大規模放量。 |
   | 🟢 | **鈀 (Pd)** | 1,508 (5/9) | 1,408 (6/8) | **-6.63%** | 燃油車滲透率下滑,觸媒需求結構性走弱。 |
   | 🟢 | **銠 (Rh)** | 9,663 (5/9) | 8,600 (6/8) | **-11.0%** | 電動車快速取代燃油車,貴金屬避險買盤退場。 |
   \`\`\`

   **欄位定義**(每欄都必須填,不可省略):
     - 燈號 — 🔴(漲)/ 🟢(跌)/ 🟡(持平,|漲跌| < 1%)
     - 金屬品種 — \`**中文名 (英文代碼)**\`
     - **上月報價** — 取 \`{{pm_monthly_changes}}\` anchor 區塊每行「一個月前」那個價(箭頭左側,千分位逗號)+ **括號標日期** \`13,150 (5/9)\`
     - **本月最新報價** — 取 \`{{pm_monthly_changes}}\` anchor 區塊每行「最新」那個價(箭頭右側)+ **括號標日期** \`13,819 (6/1)\`
...
═══ JSON 落地段 ═══
**重要**:full_content 必須是「**完整可獨立閱讀的 markdown 全文**」,**不可以**用「詳見上方 markdown 報告全文」或「請參考上文」等簡寫偷懶。落地的這份 JSON 之後會獨立顯示在 PM 平台的「月報草稿」頁,user 看不到 prompt 上方的 markdown,只看 full_content。所以必須把完整的 5 個段落(上月總覽 / 11 金屬月漲跌 / 重大事件 / 本月展望 / 採購預算影響)全部塞進 full_content,跟上方 markdown 內容一致(可重複,寧可冗長也別省略)。
...
\`\`\``;
  ...
  return {
    ...
    email_body: '上月金屬市場月報如下(附件 docx 為可編輯版本):\n\n{{ai_response}}',
    status: 'paused',
    pipeline_json: JSON.stringify(pipeline),
  };
}
...
// ── Patch 既有 [PM] 每日金屬日報:加 {{pm_current_prices}} placeholder + anchor 紀律段 ──
// 問題:LLM 沒拿到「當日 11 金屬實際報價」當 anchor,用訓練資料記憶的 2024 年金屬價推預測,
// 7 天 forecast 偏離當下實際價 50%+(實測:現價 4598 USD/oz 的 AU 預測 2465)。
// marker = `pm_current_prices`,沒這字串就 force update prompt(整段重寫)
async function patchExistingDailyReportPriceAnchor(db) {
  const newSeed = buildDailyReportTask(undefined, {});
  let row;
  try {
    row = await db.prepare(`SELECT id, name, prompt FROM scheduled_tasks WHERE name='[PM] 每日金屬日報'`).get();
  } catch (e) {
    console.warn('[PMScheduledTaskSeed] patch daily price_anchor select failed:', e.message);
    return;
  }
  if (!row) return;
  const id = row.id || row.ID;
  let promptStr = row.prompt || row.PROMPT;
  if (promptStr && typeof promptStr !== 'string' && promptStr.toString) promptStr = promptStr.toString();
  if (!!promptStr && /pm_current_prices/.test(promptStr)) return; // 已升級

  try {
    await db.prepare(`UPDATE scheduled_tasks SET prompt=?, updated_at=SYSTIMESTAMP WHERE id=?`)
      .run(newSeed.prompt, id);
    console.log(`[PMScheduledTaskSeed] Upgraded "[PM] 每日金屬日報" #${id}: 加 {{pm_current_prices}} placeholder + anchor 紀律段`);
  } catch (e) {
    console.warn(`[PMScheduledTaskSeed] patch daily price_anchor update #${id} failed:`, e.message);
  }
}
...
async function patchExistingNewsAddUrlCheck(db, kbMap) {
  ...
  for (const name of targets) {
    let row;
    try {
      row = await db.prepare(`SELECT id, pipeline_json FROM scheduled_tasks WHERE name=?`).get(name);
    } catch (e) {
      console.warn(`[PMScheduledTaskSeed] patch url_check ${name} select failed:`, e.message);
      continue;
    }
    if (!row) continue;
    const id = row.id || row.ID;
...
async function autoSeedPmScheduledTasks(db, kbMap) {
  ...
  // 2026-04-29:patch 每日金屬日報 prompt 加 {{pm_current_prices}} placeholder + anchor 紀律
  // 解決 LLM 用訓練資料記憶亂飄(現價 4598 預測 2465)
  await patchExistingDailyReportPriceAnchor(db);
