...
2. **11 金屬週漲跌幅統計與排序 — 必須輸出完整 markdown 表格(這是採購最看重的欄位,不能省)**

   表格上方先寫:
     - **領漲板塊**:前 3 名
     - **領跌板塊**:後 3 名

   接著用以下 markdown 表格格式(11 個金屬全部列出,按漲幅由高到低排序):

   \`\`\`
   | 排名 | 燈號 | 金屬品種 | 上週收盤價 | 週漲跌幅 | 主要驅動因素與市場現況 |
   | :---: | :---: | :--- | ---: | :---: | :--- |
   | **1** | 🔴 | **白銀 (Ag)** | 32.45 | **+11.4%** | 避險買盤與光電轉型需求雙擊,滬銀單週飆升。 |
   | **2** | 🔴 | **錫 (Sn)** | 31,250 | **+8.5%** | 印尼實體斷供疑慮發酵,引發短線資金擠倉逼空行情。 |
   | ... | ... | ... | ... | ... | ... |
   \`\`\`
   ...
   **欄位定義**(每欄都必須填,不可省略):
     ...
     - 上週收盤價 — 取 \`{{pm_weekly_changes}}\` anchor 區塊中「前一個價格」(箭頭左側那個價,單位 USD),數字千分位逗號,不要加單位符號。例:\`9,250\` 或 \`32.45\`
     - 週漲跌幅 — \`**+5.2%**\` 或 \`**-2.8%**\`(粗體含正負號),必須用 \`{{pm_weekly_changes}}\` 提供的實際數字
     - 主要驅動因素 — 一句話 30-60 字說明該金屬本週主漲/跌動能(地緣 / 政策 / 庫存 / 供需 / 終端需求)

   11 個金屬必須齊:CU(銅)/ AL(鋁)/ NI(鎳)/ ZN(鋅)/ PB(鉛)/ SN(錫)/ AU(金)/ AG(銀)/ PT(鉑)/ PD(鈀)/ RH(銠)
...
2. **11 金屬月漲跌統計與排序 — 必須輸出完整 markdown 表格(這是採購最看重的欄位,不能省)**

   表格上方先寫:
     - **Top 3 漲幅**
     - **Top 3 跌幅**

   接著用以下 markdown 表格格式(11 個金屬全部列出,按月漲幅由高到低排序):

   \`\`\`
   | 排名 | 燈號 | 金屬品種 | 上月收盤價 | 月漲跌幅 | 主要驅動因素與市場現況 |
   | :---: | :---: | :--- | ---: | :---: | :--- |
   | **1** | 🔴 | **白銀 (Ag)** | 28.50 | **+18.5%** | 光電產業需求 + 避險買盤共振,月線突破前高。 |
   | **2** | 🔴 | **錫 (Sn)** | 28,800 | **+12.3%** | 印尼出口限制延續整月,LME 庫存創新低引發逼空。 |
   | ... | ... | ... | ... | ... | ... |
   \`\`\`
   ...
   **欄位定義**(每欄都必須填,不可省略):
     ...
     - 燈號 — 🔴(漲)/ 🟢(跌)/ 🟡(持平,|漲跌| < 1%)
     - 金屬品種 — \`**中文名 (英文代碼)**\`
     - 上月收盤價 — 取 \`{{pm_monthly_changes}}\` anchor 區塊中「前一個價格」(箭頭左側那個價,單位 USD),數字千分位逗號,不要加單位符號
     - 月漲跌幅 — \`**+18.5%**\` 或 \`**-5.2%**\`(粗體含正負號),必須用 \`{{pm_monthly_changes}}\` 提供的實際數字
