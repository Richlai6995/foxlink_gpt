...
const FIELD_META = {
  Baseline: [
    ...
    ['SMT_POINT_UNIT_PRICE', 'SMT 點數單價', 'USD/點(SimplifiedLine calc_mode=SMT_POINTS 用;支援 6 位小數)', 'SIMPLIFIED'],
  ...
  SimplifiedLine: [
    ...
    ['LINE_GROUP', '群組', 'MATERIAL(材料·有BOM時自動改用rollup)/ PROCESS(製程)/ LOSS(損耗)/ CONSUM(耗材·不被 BOM 取代)', 'SIMPLIFIED'],
    ['COST_PER_UNIT_USD', '每台成本', 'USD/unit(calc_mode=AMOUNT 時生效;其他模式為系統算出)', 'SIMPLIFIED'],
    ['CALC_MODE', '計算模式', 'AMOUNT(手填·預設)/ YIELD_PCT(Σ勾選基數×%)/ MACRO(段站製程試算)/ SMT_POINTS(Σ點數×單價)', ''],
    ['YIELD_PCT', '損耗率', '小數口徑:0.005 = 0.5%(YIELD_PCT 模式用)', ''],
    ['YIELD_BASIS_JSON', '損耗基數勾選', 'JSON 陣列:line_code 或 BOM_MATERIAL_ALL/EE/ME/PKG — 例 ["BOM_MATERIAL_EE","SMT"]', ''],
    ['MACRO_CODE', '綁定製程段', 'MACRO 模式:對應 MacroProcess 分頁的 MACRO_CODE', ''],
    ['SORT_ORDER', '排序', '數字(YIELD_PCT 基數只能取排序在前的線)', ''],
  ],
  MacroProcess: [
    ['MACRO_CODE', '製程段代碼', '如 FATP / GLUE_ATE / SMT_SEG(SimplifiedLine.MACRO_CODE 綁這裡)', ''],
    ['NAME', '段名', '如 FATP 組裝', ''],
    ['UPH', '段級 UPH', '無站明細時用段級參數粗算(有站明細 = Σ 站,段級可空)', ''],
    ['DL_HEADCOUNT', '段級 DL 人數', '同上', ''],
    ['WORK_TIME_SEC', '段級工時(秒)', 'UPH 空時用工時制:DL × 秒 ÷3600 × DL時薪', ''],
    ['PROCESS_YIELD_PCT', '段良率', '0~1(參考;損耗成本由 YIELD_PCT 線算)', ''],
  ],
  MacroStation: [
    ['MACRO_CODE', '所屬段', '對應 MacroProcess 分頁(段須先定義,否則該站跳過)', ''],
    ['STATION_CODE', '站代碼', '如 B301(段內唯一)', ''],
    ['NAME', '站名', '', ''],
    ['NUM_STATIONS', '站數', '預設 1', ''],
    ['UPH', '站 UPH', '站成本 = DL × DL時薪 ÷ UPH', ''],
    ['YIELD_PCT', '站良率', '0~1', ''],
    ['WORK_TIME_SEC', '工時(秒)', 'UPH 空時用工時制', ''],
    ['DL_HEADCOUNT', 'DL 人數', '預設 1', ''],
    ['SORT_ORDER', '排序', '數字', ''],
  ],
  SmtPoint: [
    ['BOARD_CODE', '板代碼', '如 HARVARD_SENSOR(一板多列)', ''],
    ['CATEGORY_CODE', '元件類別', 'C/P、R、U、D、L、J…', ''],
    ['PCS', '元件數', '顆', ''],
    ['TRANSFER_POINT', '轉換點數', 'SMT 成本 = Σ(轉換點數 × Baseline.SMT_POINT_UNIT_PRICE)', ''],
  ],
  QtyScenario: [
...
const SHEET_GUIDE = {
  ...
  MacroProcess: ['製程段(SIMPLIFIED 選填 · 段→站製程試算)', 'EPM / IE', [
    '一列 = 一個製程段(如 FATP / GLUE_ATE);SimplifiedLine 把 CALC_MODE 設 MACRO 並填 MACRO_CODE 綁定後,該線成本改由段站參數計算。',
    '有站明細(MacroStation 分頁)= Σ 站(DL × DL時薪 ÷ UPH);無站 = 用本頁段級 UPH/DL 粗算。',
    '不同專案製程不同 → 自由開段開站,不用改系統。',
  ]],
  MacroStation: ['製程站明細(SIMPLIFIED 選填 · 掛在段下)', 'EPM / IE', [
    '一列 = 一站(對齊真實 FATP / SMA+BFT sheet:站數 / UPH / Yield / 工時 / DL)。',
    'MACRO_CODE 須先在 MacroProcess 分頁定義,否則該站匯入時跳過(有警告)。',
    'UPH 有值 → DL×時薪÷UPH;UPH 空 → 工時制 DL×秒÷3600×時薪。',
  ]],
  SmtPoint: ['SMT 點數(SIMPLIFIED 選填 · 點數制報價)', 'EPM / SMT', [
    '一列 = 板 × 元件類別的轉換點數(對齊 SMT Quotation sheet)。',
    'SimplifiedLine 把 SMT 線 CALC_MODE 設 SMT_POINTS → 成本 = Σ(轉換點數 × Baseline.SMT_POINT_UNIT_PRICE)。',
  ]],
