    ...
    blocks: [
        ...
        blocks: [
          { type: 'para', text: '為了讓對話內容能完整展開、減少操作時忽上忽下,手機介面 2026-05 改成 **Gemini App 風格**:上方 Topbar 大幅精簡,常用控制項全部下放到輸入區下方的「控制列」,跟輸入框一起放在底部。' },
          {
            ...
            blocks: [
              ...
              { type: 'tip', text: '控制列左半(`+` / 模型 / 齒輪)是「配置」類功能,右半(麥克風 / 送出)是「輸入/送出」類,中間刻意留出空白,避免錄音結束想點 mic 時誤碰齒輪。' },
            ],
          },
          {
            type: 'subsection',
            ...
            blocks: [
              ...
              { type: 'note', text: '錄音中 mic icon 是紅色脈動。錄音上限 60 秒,到時自動停。中途想取消可再點一下 mic。' },
            ],
          },
          {
            type: 'subsection',
      ...
      },
      {
        ...
        blocks: [
          ...
          { type: 'tip', text: '**雙通道發送(2026-05 強化):驗證碼會「同時」發到您的 Webex 和 Email**,兩個通道都是同一個 6 位數碼,任一收到輸入即可。手機沒裝 Webex App 的同仁可看 Email,在公司用 Webex 比較順手的就用 Webex,各取所需。' },
          {
            type: 'steps',
            items: [
              { title: '輸入帳號密碼後,系統判斷您不在內網範圍' },
              { title: '同時透過 Webex 私訊 + Email 寄出 4 位驗證碼', desc: '5 分鐘內有效;同時告訴您「來源 IP」讓您確認是不是自己操作' },
              { title: '前端切換到驗證碼輸入畫面', desc: '可按「重新發送」(60 秒冷卻)' },
              { title: '輸入正確驗證碼即完成登入,系統會在您的瀏覽器發一個信任 cookie(30 天)' },
              { title: '同一台裝置 30 天內再登入直接通過,**不必重新驗證碼**(即使換網路 / IP 變動)' },
            ],
          },
          { type: 'note', text: '**沒收到驗證碼怎麼辦?** 1) **檢查 Email**(Webex 沒收到也可從 Email 拿)2) 確認 Webex App 開著且通知開啟 3) 檢查跟 Cortex Bot 的 1:1 對話 4) 若兩個通道都沒收到,登入畫面會顯示「事件代碼」,把它告訴系統管理員可協助查詢' },
          { type: 'tip', text: '**Admin 帳號**從外網登入跟一般使用者一樣需走 Webex / Email 驗證碼;通過後同樣享 30 天信任裝置免重認。若資安政策變嚴可改用 IP 白名單(EXTERNAL_ALLOWED_IPS)限定 admin 連線來源。' },
        ],
      },
      {
        type: 'subsection',
        ...
        blocks: [
          { type: 'para', text: '完成 Webex 驗證後,您的瀏覽器 / 手機會被登記為「信任裝置」,30 天內免重新驗證碼。**換網路 / 4G 切 Wi-Fi / ISP 重撥 / 出差移動辦公,IP 變動都不受影響**,只有換瀏覽器 / 清 cookie / 改密碼會視為新裝置。' },
          {
            type: 'subsection',
            title: '進入「我的裝置」頁面',
            blocks: [
              { type: 'para', text: '在 Cortex 左下角狀態列點 **📱 圖示**(在「鑰匙」按鈕旁邊),即可看到自己所有信任裝置。' },
              {
                type: 'list',
                items: [
      ...
      },
      {
        ...
        blocks: [
          ...
          { type: 'tip', text: '**登出不會清掉信任裝置**。同一台裝置下次登入仍可在 30 天內免驗證碼。要徹底登出該裝置請用「我的裝置」頁面移除。' },
        ],
      },
    ],
  },
...

  {
    ...
    blocks: [
      ...
      },
      {
        ...
        blocks: [
          { type: 'para', text: '頂部工具列「技能」按鈕右邊有一個「純對話」toggle，點一下就會**完全停用所有工具、知識庫、技能、API 連接器**，讓 AI 直接以最單純的對話方式回應。' },
          {
            type: 'list',
            items: [
          ...
          { type: 'tip', text: '純對話模式下 AI 回應通常會更快（因為 server 不需要掃過所有工具與知識庫做自動匹配）。如果發現某些功能（如查公司知識庫、執行技能）「沒反應」，請先確認頂部「純對話」是否被誤啟用。' },
          { type: 'note', text: '純對話模式不會清除您原本掛載的技能或選擇的知識庫，只是**這次對話暫時不引用**。關閉純對話後，您之前的選擇全部還在，繼續沿用。' },
        ],
      },
      {
        type: 'subsection',
...

  {
    ...
    blocks: [
      ...
      },
      {
        ...
        title: '隱藏不要的工具（個人化精簡）',
        blocks: [
          { type: 'para', text: '工具列各下拉選單（MCP / API / 知識庫 / 技能 / ERP）中，每個項目右側都有 👁 眼睛圖示。點一下可將該工具收進「已隱藏」摺疊區，讓選單看起來更清爽 — 同時系統不會再把這個工具送給 AI。' },
          {
            type: 'list',
            items: [
          ...
          { type: 'note', text: '適合場景：某些工具您完全用不到（例如採購人員看不到「捷運即時動態」、業務人員看不到「程式碼優化」技能），隱藏起來就不會干擾 AI 的路由判斷，選單也清爽，AI 回應也更快。' },
        ],
      },
      {
        type: 'subsection',
        ...
        blocks: [
          {
            ...
            rows: [
              ...
              ['點 👁 隱藏某些工具', '不論 Auto / Explicit，這些工具都從候選清單剃除', '授權工具減去您隱藏的'],
            ],
          },
          { type: 'tip', text: '最精確的方式：下拉選單明確勾選目標工具 + 關閉不需要的類別開關。最省力的方式：所有工具設定好 Tags，讓系統 TAG 路由自動判斷。常用不到的工具點 👁 隱藏起來，可降低 AI 判斷負擔、加快回應。' },
        ],
      },
      {
        type: 'subsection',
...

  {
    ...
    blocks: [
        ...
        blocks: [
          {
            ...
            items: [
              ...
              { title: '點選「新增任務」按鈕' },
              { title: '填寫任務名稱與執行頻率', desc: '選擇每日 / 每週 / 每月 / 每隔幾小時 / 多時段等頻率，並設定幾點幾分執行' },
      ...
      },
      {
        ...
        title: '設定執行頻率',
        blocks: [
          { type: 'para', text: '在「排程設定」頁籤可選擇任務的觸發頻率，畫面下方會即時預估「接下來 5 次執行時間」供您確認：' },
          {
            type: 'table',
            headers: ['頻率', '說明', '適用場景'],
            rows: [
              ['每日', '每天固定時間執行一次', '每日新聞摘要、當日工作提醒'],
              ['每週', '每週指定星期幾固定時間執行', '週報、週一行情回顧'],
              ['每月', '每月指定日期固定時間執行（建議 1–28 號避開月底落差）', '月結報表、月度統計'],
              ['每隔 N 小時', '從整點起算，每隔 N 小時執行一次（N 為 1–23）', '盤中行情每 4 小時更新、定期巡檢'],
              ['多時段', '一天內多個指定時間點各執行一次（最多 12 個）', '早中晚三次匯率快訊'],
              ['進階 Cron', '直接填寫 5 欄位 Cron 運算式（分 時 日 月 週幾），可表達複雜規則', '只在工作日 9–17 點每 15 分執行'],
            ],
          },
          {
            type: 'code',
          ...
          { type: 'tip', text: '不確定 Cron 怎麼寫時，「多時段」可涵蓋大多數「一天跑好幾次」的需求，不需要記 Cron 語法。' },
        ],
      },
      {
        type: 'subsection',
      ...
      },
      {
        ...
        blocks: [
          ...
          { type: 'para', text: '**套用文件範本**：若想讓產出的檔案有固定版型（公司報告格式、表頭、樣式），可在 AI 設定頁的「輸出範本」欄位選擇已建立的範本。排程執行後，AI 的回應會自動解析為 JSON 套進範本生成檔案；也可在 Prompt 中用 `{{template:範本ID}}` 指定。' },
        ],
      },
      {
        type: 'subsection',
        ...
        blocks: [
          { type: 'para', text: '撰寫排程 Prompt 時，可以直接在文字中引用已建立的技能（Skill）、知識庫（KB）和 API 連接器（DIFY / REST），系統執行時會自動呼叫對應的工具並將結果注入給 AI 分析。' },
          {
            type: 'table',
            headers: ['語法', '說明', '範例'],
            rows: [
              ...
              ['{{dify:連接器名稱}}', '呼叫指定的 DIFY / REST API 連接器，將回應注入背景。預設以任務名稱當查詢字串', '{{dify:工單查詢}}'],
              ['{{dify:連接器名稱 query="..."}}', '同上，但明確指定查詢字串（覆蓋預設）', '{{dify:工單查詢 query="本日逾期工單"}}'],
            ],
          },
          {
            type: 'code',
          ...
          { type: 'tip', text: '撰寫 Prompt 時，輸入 `{{skill:` 或 `{{kb:` 後，系統會自動彈出技能 / 知識庫名稱選單供快速選取，也可用斜線 `/` 觸發同樣的選單。API 連接器則可在「工具」頁籤的「可用 API 連接器」清單點「插入」加入。' },
          { type: 'note', text: 'API 連接器需先由系統管理員在「知識庫市集 → DIFY / API 連接器」建立並分享給您，才會出現在清單與可被引用。權限與您在對話中直接使用該連接器時相同。' },
        ],
      },
      {
        type: 'subsection',
        ...
        blocks: [
          ...
          { type: 'para', text: '在任務編輯頁切換至「**Pipeline 管線**」頁籤，點「+ 新增節點」加入步驟。' },
          {
            ...
            rows: [
              ...
              ['**API 連接器**', '🔗 天藍', '呼叫 DIFY / REST API 連接器，query 預設接上一步輸出，可加固定參數', '工單系統查詢、外部資料 API'],
              ['AI 追加（AI）', '🤖 藍色', '讓 AI 對前一步的結果再做分析/整理，需撰寫 Prompt', '整合多個來源後做二次摘要'],
              ['生成檔案', '📤 靛色', '將前一步 AI 輸出生成特定格式檔案', '輸出 Excel / PDF / PPT / Word'],
              ['**AI 戰情**', '📊 橘色', '呼叫 AI 戰情 design 執行查詢，結果輸出為 Excel 或丟給下游節點', '每日庫存日報、超額庫存 TOP 10、Multi-Org 報表'],
              ['**合併 Excel**', '📁 青色', '把多個 AI 戰情節點的結果合併成單一 Excel（多 sheet 或單 sheet）', '週報整合（庫存 + WIP + PO 多個 design 一張表）'],
              ['**知識庫寫入**', '📚 翠綠', '把前一步的 JSON 結果切塊向量化後寫入指定知識庫', '每日把新聞 / 公告自動收進 KB'],
              ['**警示**', '🚨 紅色', '依門檻判斷（超標 / 與歷史均值比較等），符合條件才發 Email / Webex / Webhook 通知', '庫存超標警示、達交率異常通知'],
              ['**DB 寫入**（Admin）', '📥 灰色', '把前一步的 JSON 結果寫入白名單資料表（僅系統管理員可建立）', '排程結果落地到 ERP 中介表'],
              ['條件判斷', '🌿 玫瑰色', '根據前一步輸出進行分支（if/else），支援 AI 判斷或文字比對', '根據異常數量高低分流'],
              ['並行執行', '⑁ 青色', '將多個子步驟同時執行，加速多路查詢', '同時查多個知識庫、多個 API'],
            ],
          },
          { type: 'note', text: '「知識庫寫入」「警示」「DB 寫入」屬於進階節點，會產生實際的資料寫入或對外通知，建議先用單次手動執行（任務列表的 ▶ 立即執行）驗證無誤再啟用排程。其中「DB 寫入」僅系統管理員可建立。' },
          ...
          { type: 'para', text: '部分節點（如 AI 戰情、合併 Excel）另有「**必要節點**」勾選：勾選後該節點失敗就 fail-fast 中斷整個 Pipeline；不勾選時失敗不中斷，但寄出的 Email 主旨 / 內文會標上 ⚠️ 提示某些節點未成功，方便您一眼看出這份報告不完整。' },
          {
            type: 'code',
      ...
      },
      {
        ...
        blocks: [
          { type: 'para', text: '排程任務可在 Pipeline 內加入「API 連接器」節點，自動呼叫已建好的 DIFY 或 REST API 連接器，把回應丟給下游節點（例如交給「AI 追加」整理、或寫進報告）。適合需要從外部系統（工單、HR、自建服務）定期拉資料的場景。' },
          {
            type: 'steps',
            items: [
              { title: '在 Pipeline 加「API 連接器」節點', desc: '從「+ 新增節點」選單挑「API 連接器」' },
              { title: '下拉選連接器', desc: '清單只列出您有權限的連接器（對齊您在對話中能用的範圍）；🤖 為 DIFY、🔌 為 REST API' },
              { title: '設定查詢字串（query）', desc: '預設帶上一節點輸出 {{ai_output}}，也可改填固定文字或其他變數（{{date}} / {{node_X_output}}）' },
              { title: '填寫固定參數（如有）', desc: '若該連接器定義了需手動提供的參數，會逐欄列出供填寫，可用變數插值' },
            ],
          },
          { type: 'note', text: '連接器若設定了「必填參數」而排程沒填，該節點會被標為失敗（不會假裝成功往下送錯資料）。建立後請用「立即執行」驗證一次。' },
          { type: 'tip', text: '只是想把連接器結果注入主 Prompt、不需要多步驟時，直接在 Prompt 用 `{{dify:連接器名稱}}` 語法更簡單，不必建 Pipeline 節點。' },
        ],
      },
      {
        type: 'subsection',
        ...
        blocks: [
          { type: 'para', text: '排程任務可在 Pipeline 內加入「AI 戰情」節點,自動呼叫已建好的 AI 戰情 design(例如「合理庫存主表查詢」),把查詢結果產出 Excel 寄到您的信箱。完全不需要每天手動到 AI 戰情頁去查。' },
          { type: 'note', text: '排程觸發時的權限完全等同您本人手點 AI 戰情查詢：design 必須是您能看的、資料政策、Multi-Org 範圍全部都會檢查。看不到 design 就不會列在下拉,跑出來的資料也只會在您權限範圍內。' },
          {
            type: 'steps',
            items: [
              { title: '在 Pipeline 加「AI 戰情」節點', desc: '從「+ 新增節點」選單挑「AI 戰情」' },
              { title: '下拉選 Design', desc: '清單只列出您有權限的 design(對齊 AI 戰情頁的分享範圍)' },
              { title: '撰寫查詢問題', desc: '支援 {{date}} / {{ai_output}} / {{node_X_output}} 插值。例如：「{{date}} 各 ORG_CODE 超額庫存金額 TOP 10」' },
              { title: '選輸出格式', desc: 'Excel = 直接落檔給 Email 附件;JSON 文字 = 給下游 AI 追加節點接;不落檔 = 只供「合併 Excel」節點使用' },
              { title: '點「Try it 立即試跑」驗證', desc: '即時看到 SQL 與前 100 筆資料預覽,確認結果正確再儲存' },
            ],
          },
          { type: 'tip', text: '「Try it」試跑後,如果該 design 有 Multi-Org 範圍(例如您可以看 GAD、Z4E、TPE 三家),畫面下方會顯示「🎯 限縮 Multi-Org 範圍」chip,可勾選只要查其中一兩家。沒勾 = 用您完整的權限範圍。' },
        ],
      },
      {
        type: 'subsection',
        ...
        blocks: [
          { type: 'para', text: '如果您想每週收到一個 Excel,內含「庫存日報」「WIP 異常」「PO 達交率」三張工作表,可用「合併 Excel」節點把多個 AI 戰情節點的結果合併。' },
          {
            type: 'steps',
            items: [
              { title: '建立多個 AI 戰情節點', desc: '每個對應一個 design,輸出格式選「不落檔(僅供下游合併用)」避免重複產檔' },
              { title: '在後面加「合併 Excel」節點', desc: '勾選要合併的 AI 戰情節點' },
              { title: '選合併模式', desc: '多 sheet = 每個來源一張工作表;single sheet = 全部接在同一張並加 _source 標記欄' },
              { title: '填輸出檔名', desc: '例如 週報合併_{{date}}.xlsx' },
            ],
          },
          { type: 'note', text: '某個 AI 戰情節點失敗時(例如該 design 被收回權限),合併 Excel 仍會產出,失敗的工作表會標 `_MISSING` 並在 Email 主旨加 ⚠️ 提示。其他成功的節點不會受影響。' },
        ],
      },
      {
        type: 'subsection',
        title: '分享任務給其他使用者',
        blocks: [
          { type: 'para', text: '排程任務可以分享給其他使用者協助維護(例如管理員幫部門建好排程後,讓部門同仁自己微調 Prompt 或時段)。' },
          { type: 'para', text: '在任務列表上點 row 右側的 **🔗 紫色分享圖示**(只有任務擁有者或管理員看得到),或進入編輯任務頁右上角點「分享」按鈕,即可開啟分享設定。' },
          {
            type: 'table',
            headers: ['權限等級', '可以做什麼', '不能做什麼'],
            rows: [
              ['**檢視權限(use)**', '看設定、看執行歷史、下載產出檔', '不能改任何欄位、不能立刻執行、不能啟用停用'],
              ['**開發權限(develop)**', '可改 Prompt / Pipeline / 排程時段 / 收件人、立刻執行、啟用停用', '不能刪除任務、不能改分享設定'],
            ],
          },
          { type: 'tip', text: '分享對象支援 7 種:使用者、角色、部門、廠區、利潤中心、事業處、事業群。例如分享給「品保部」整個部門,部門內成員都看得到。' },
          { type: 'note', text: '**重要 — 執行身份**:無論被分享者拿到 develop 改了什麼內容,排程觸發時的執行身份永遠是「任務擁有者(建立者)」。也就是說,擁有者建的 AI 戰情 design 即便被分享者沒權限看,排程跑出來的 Excel 還是會包含完整資料寄到收件人信箱。這是設計上的權限模型,讓管理員能放心代建排程給沒 AI 戰情權限的同仁定期收報表。' },
          { type: 'note', text: '**特殊限制**:含有 db_write / kb_write / 警示節點的排程任務,僅允許「檢視權限」分享,不能分享「開發權限」(避免被分享者改 column_mapping 等敏感欄位偷用擁有者權限寫入資料)。如要分享,請先把這些節點刪除,或讓被分享者另外建一個新任務。' },
        ],
      },
      {
        type: 'subsection',
        title: '排程任務數量上限',
        blocks: [
          { type: 'para', text: '系統預設每人最多可建立 **10 個排程任務**。如有需要更多,請聯絡系統管理員,可針對您的帳號個別設定上限,或調整全公司預設值。管理員身分不受此上限約束。' },
        ],
      },
    ],
  },
...

  {
    ...
    blocks: [
      { type: 'para', text: '技能（Skill）是可以掌載到對話的自訂模組，能讓 AI 具備特定領域的專業知識、固定指令或對接外部服務的能力。例如，掛載「專業術語翻譯」技能後，每次對話 AI 會自動以該行業的標準用語進行翻譯。' },
      {
        ...
        blocks: [
          { type: 'para', text: '若技能或 MCP 工具回傳的內容**本身就是成品**（例如完整的 markdown 表格、HTML 報表），管理員可在編輯介面勾選「啟用自動 Passthrough」。命中後的內容會**直接渲染給使用者**，跳過 LLM 整理 — token 大幅節省、回應更即時、原始格式不被 AI 改寫。' },
          { type: 'tip', text: 'Markdown artifact 提供「複製 Markdown」按鈕；HTML 走 sandboxed iframe 顯示，僅限下載。**若想讓 AI 進一步分析 artifact 內容,請下載後在新對話以附件上傳** — Passthrough 內容不會自動回流到 LLM context。' },
          { type: 'note', text: 'Passthrough 內容不經 LLM,**敏感詞掃描也會跳過**。管理員應只對受信任來源的工具開啟此功能。' },
        ],
      },
      {
        type: 'subsection',
