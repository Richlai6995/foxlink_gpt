module.exports = [
  {
    ...
    sidebar_label: '系統介紹',
    blocks: [
      { type: 'para', text: 'Foxlink GPT to Cortex 是正崴精密工業內部專屬的 AI 智慧助理平台，同時整合 **Azure OpenAI (AOAI)** 與 **Google Gemini** 兩大語言模型體系，提供流暢的多語言對話、文件深度分析、多媒體處理、工具調用及自動化排程等功能，協助同仁大幅提升日常工作效率。' },
    ...
    blocks: [
      ...
      },
      {
        ...
        blocks: [
          ...
          { type: 'para', text: '詳細的圖片生成與修圖操作說明，請參閱本文件的「圖片生成與修圖」章節。' },
        ],
      },
    ],
  },

  {
    ...
    blocks: [
      { type: 'para', text: '系統同時支援 **Azure OpenAI（AOAI）** 及 **Google Gemini** 兩大平台的多種模型，由左側邊欄的模型下拉選單切換。不同模型在能力、速度、費用上各有差異，請依需求選擇。' },
      {
        type: 'card_grid',
        cols: 2,
        items: [
      ...
      { type: 'note', text: '**模型清單由系統管理員統一維護，**實際可用模型以畫面下拉選單為準，後續將陸續新增更多 AOAI 及 Gemini 模型版本。模型選擇會保存在您的瀏覽器，下次開啟仍會維持上次的選擇；切換模型不會影響當前對話歷史記錄。' },
    ...
    blocks: [
      {
        ...
        blocks: [
          ...
          { type: 'note', text: '不支援上傳影片檔案。單檔最大限制為 50MB（依管理員設定可能有所不同）。' },
        ],
      },
      {
        type: 'subsection',
    ...
    blocks: [
      ...
      },
      {
        ...
        blocks: [
          ...
          { type: 'para', text: '此外，您也可以點選各開關圖示旁的下拉箭頭，**明確指定**要使用哪幾個知識庫 / MCP 伺服器 / DIFY 知識庫，而非讓系統自動選擇全部。' },
        ],
      },
      {
        type: 'subsection',
      ...
      },
      {
        ...
        blocks: [
          { type: 'para', text: '每次送出訊息時，系統依以下步驟決定傳哪些工具給 AI：' },
          {
            type: 'steps',
            items: [
              { title: '依存取權限載入可用工具', desc: 'MCP 依 mcp_access 授權、DIFY KB 依 dify_access 授權、自建 KB 依 creator/公開/kb_access 授權 — 只載入您有權使用的工具' },
...

  {
    ...
    blocks: [
      ...
      },
      {
        ...
        title: '個人分享技能給特定對象',
        blocks: [
          { type: 'para', text: '不想公開給全員、只想分享給特定同事？可在技能卡片選單點「分享」，選擇分享對象（使用者 / 角色 / 部門）後設定「使用」或「管理」權限，對方可在技能市集的「分享給我」區找到此技能。' },
        ],
      },
      {
        type: 'subsection',
      ...
      },
      {
        ...
        blocks: [
          { type: 'para', text: '技能可以綁定特定的自建知識庫與 DIFY 知識庫，讓掛載技能時自動啟用相關知識庫，無需使用者手動選取。' },
          {
            type: 'table',
            headers: ['模式', '說明'],
            rows: [
      ...
      },
      {
        ...
        title: 'TAG 自動路由',
        blocks: [
          { type: 'para', text: '系統使用 TAG 標籤機制自動判斷每則訊息應啟用哪些工具、知識庫與技能，無需使用者手動勾選。每個 MCP 伺服器、DIFY 知識庫、自建知識庫及技能都可設定標籤（Tags），系統流程如下：' },
          {
            type: 'steps',
            items: [
      ...
      },
      {
        ...
        blocks: [
          ...
          { type: 'para', text: '在 System Prompt 中使用 {{變數名稱}} 語法引用變數，例如：' },
          { type: 'code', text: '你是一位專業翻譯員，請將使用者的文字翻譯為 {{target_language}}。\n翻譯風格：{{style}}' },
          { type: 'tip', text: '掛載技能時若該技能定義了輸入變數，系統會彈出表單讓使用者填寫，填寫後的值會存在該對話的技能設定中。' },
        ],
      },
      {
        type: 'subsection',
        ...
        blocks: [
          { type: 'para', text: 'Code 類型技能可以定義 **Tool Schema**（Gemini Function Declaration），讓 AI 在對話中自動判斷是否需要呼叫該程式技能，而非每次都觸發。' },
          { type: 'para', text: '例如，一個「股價查詢」Code 技能定義了 Tool Schema，AI 收到「台積電股價多少」時會自動呼叫，但收到「今天天氣如何」則不會觸發。這等同於 Gemini 的 Function Calling 機制。' },
          { type: 'note', text: '未定義 Tool Schema 的 Code 技能仍按原有的 inject/answer 端點模式運作，不受影響。' },
        ],
      },
      {
        type: 'subsection',
        ...
        blocks: [
          { type: 'para', text: '技能可以定義 JSON 格式的輸出結構（Output Schema），指引 AI 以固定的 JSON 格式回覆。適合需要結構化資料的場景，如自動產生工單、匯出報表欄位等。' },
          {
            type: 'code',
...

  {
    ...
    blocks: [
        ...
        blocks: [
          {
            ...
            items: [
              ...
              { title: '選擇 Embedding 維度', desc: '768 維適合大多數情況；1536 / 3072 精度更高但費用也較高，且建立後無法更改' },
      ...
      },
      {
        ...
        title: '上傳文件',
        blocks: [
          { type: 'para', text: '進入知識庫後，點選「文件」頁籤，拖曳或點選上傳區域選擇檔案。支援格式：**PDF · DOCX · PPTX · XLSX · TXT · CSV · JPG · PNG · GIF · WEBP**（單檔最大 200 MB）。' },
          {
            type: 'steps',
            items: [
      ...
      },
      {
        ...
        blocks: [
          ...
          { type: 'tip', text: '可以同時掛載「自建知識庫」＋「DIFY 知識庫」＋「MCP 工具」，AI 會綜合所有來源回答。此外，若知識庫設有標籤（Tags），系統會透過 TAG 自動路由機制，根據訊息內容自動判斷是否啟用對應知識庫。' },
        ],
      },
      {
        type: 'subsection',
      ...
      },
      {
        ...
        title: '共享知識庫',
        blocks: [
          { type: 'para', text: '進入知識庫詳情 → 點選「共享設定」頁籤，可將知識庫共享給特定使用者、角色、部門或組織單位。' },
          {
            type: 'table',
            headers: ['共享方式', '對象'],
            rows: [
