...
// 2026-04-27 Phase 5 ship 後 hotfix:更新 pm-briefing-page 加歷史價格 tab + chart
// 細節 + 詳細表 CSV 匯出;同日修了多個資料一致性 bug(metal_code 大小寫、TZ、
// metal_price_history 自動 migrate、token aggregator backfill、source stale row 等)
// 完整修法見 [docs/precious-metals-plan.md §13](../../docs/precious-metals-plan.md)
// 2026-04-28 採購需求變更 + UI 改善:
//   - 排程資料健康度面板(顯示 6 任務跑況 + silent failure 警示)
//   - 報價 banner 加缺金屬警示 + stale 視覺標記
//   - 歷史價格 tab 完整資料表搬上、圖表搬下
//   - 新聞 card 日期 + 來源網址搬左上
//   - 資料源變更:AU 改台銀 / Pt-Pd-Rh 用 JM RSS / 銀 Westmetall+Kitco 雙源
//   - 新聞源改 Mining.com RSS / MoneyDJ / SMM / 日經中文 + PGM 機構評論(WPIC + matthey)
// 2026-04-29 大重構:
//   - 新聞抓取拆 5 個 by-source task(B+E 策略,各 source 一 task)取代單一彙整 task
//   - master scrape 改純報價(11 金屬報價)— 砍掉新聞段、不發 email、寫 PM-原始資料庫
//   - 每日金屬日報 → 統一綜合分析 + 新聞精選 5-10 篇 RAG + 預測 + 寄 email
//   - 新增 {{news_seen:source:days:limit}} placeholder — LLM 抓新聞前比對已抓 url 跳過
//   - 資料保留設定(retention):7 個 entity 各自設天數 + 排程時間可調
//   - briefing 頁加宏觀指標 banner、近期警示 banner(可 ack);新聞卡「看完整內容」modal
//   - 來源篩選改 url domain match,合併同網站不同 source 字串變體
//   - RunDetailModal 顯示完整 pipeline 流程 + 預覽寫入內容 + KB url 對照 + 標示 AI 回應來源
const briefingLastModified = '2026-04-29';
...
const sections = [
  ...
  // ── 1.5. 採購每日工作中心(Phase 5 後新增的主頁)──────────────────────────
  {
    ...
    last_modified: briefingLastModified,  // 2026-04-28:健康度面板 + 缺金屬警示 + UI 改善
    ...
    blocks: [
        ...
        blocks: [
          {
            ...
            cols: 2,
            items: [
              { emoji: '🏷️', title: '1. 報價 banner(置頂常駐)', desc: '依「我的偏好」顯示關注金屬,**捲動仍可見**;[展開全部] 看 11 金屬完整清單;**今日不齊時自動橘色警示列出缺哪些金屬** + 個別金屬 stale 黃框 + 日期標記' },
              { emoji: '📈', title: '2. 宏觀指標 banner(2026-04-29 加)', desc: 'DXY / VIX / 10Y / WTI 等宏觀因子單行展示,沒資料自動隱藏;對應 pm_macro_history 最新值' },
              { emoji: '🚨', title: '3. 近期警示 banner(2026-04-29 加)', desc: '近 7 天未 ack 警示卡(MAPE 3 連發 / 價格突破 / 全網來源失效)— 可一鍵 ack;全部 ack 收成單行 ✅,沒警示時不佔位' },
              { emoji: '⭐', title: '4. 今日 AI 綜述', desc: '從每日金屬日報排程撈,前 250 字摘要 + [看完整日報] 展開全文 + 👍 thumbs feedback;**fallback 文字會動態顯示實際排程時間**' },
              { emoji: '📊', title: '5. 排程資料健康度面板', desc: '一頁看完所有 [PM] 排程任務跑況(含 by-source 5 個新聞 task);預設摺疊,有 silent failure 才展開吸引點擊' },
              { emoji: '📑', title: '6. 5 個 Tab(內容區)', desc: '[新聞列表(預設)] [歷史價格] [週報] [月報] [Prompt 審核] — Prompt 審核從原獨立頁整合進來,sidebar 不再多入口' },
              { emoji: '⚙️', title: '7. Top bar 操作', desc: '右上有 [⚙ 我的偏好] 設關注金屬 / 是否預設只看 24h、[⬇ 匯出 CSV] 下載 30 天價格' },
            ]
          },
        ]
      },
      ...
      {
        type: 'subsection',
        title: '🏷️ 報價 banner — 缺金屬警示 + stale 標記(2026-04-28 新增)',
        blocks: [
          {
            type: 'para',
            text: '原本只是橫向 11 金屬報價列。新增兩個視覺提示讓使用者**一眼辨識資料是否「今天的」**:'
          },
          {
            type: 'card_grid',
            cols: 2,
            items: [
              {
                emoji: '⚠️',
                title: '今日不齊警示(橘色條)',
                desc: '報價列上方多一行:「**2026-04-28 報價不完整 — 今日只抓到 X/11 個金屬。缺:RH (04-27)、IR (04-26)…**」+「→ 顯示其他金屬以最近一筆替代」。今日 11 金屬都齊就不顯示。'
              },
              {
                emoji: '🟨',
                title: 'Stale 卡片黃框 + 日期標',
                desc: '個別金屬若不是今天的資料(LLM 抓不到 → 回退到最近一筆),卡片**邊框轉黃** + 卡內小字標 `MM-DD`(例 `04-27`)。hover 看完整日期 + 「此金屬今日無新報價」說明。'
              },
            ]
          },
          {
            type: 'note',
            text: '為何重要:**status=ok 不等於資料真寫進來**。LLM 偶爾只生 1-2 筆 prices,銀行就會看到報價列大半變黃 + 上方警示一目了然。再去 admin 排程詳情看 row error 排查。'
          },
        ]
      },
      ...
      {
        type: 'subsection',
        title: '📊 排程資料健康度面板(2026-04-28 新增)',
        blocks: [
          {
            type: 'para',
            text: '原本問題:6 個 [PM] 排程任務 status=ok 但實際 db_write 0 inserted(LLM 抓的 url 撞 dedupe 全部 skipped、或欄位不存在 row error 整批 fail),使用者完全不知道。**這個面板就是讓「靜默失敗」浮上檯面**。'
          },
          {
            type: 'subsection',
            title: '面板內容',
            blocks: [
              {
                type: 'list',
                items: [
                  '6 個 [PM]% 排程任務各一張 card(自建 task 也會自動列入,例 [PM] 每日貴金屬行情)',
                  'card 顯示:任務名 + 排程時間 + 上次跑時間/狀態 + **目的地表 + 上次寫入統計**(`📥 pm_news: 5+ / 12↻ / 3⊘ / 0✗` = 5 inserted / 12 updated / 3 skipped / 0 errors)',
                  'card 邊框顏色:🟢 綠(今日 OK)/🟡 橘(silent failure)/⚫ 灰(暫停)',
                  '偵測「上次跑成功但 0 inserted」 → 顯示警示條:「⚠ 上次跑成功但 0 筆 inserted(N 筆被 dedupe / unique 衝突 skip)」',
                  '若 LLM 出 schema 沒的欄位(被 silent drop):card 上小字「⚠ LLM 出了 schema 沒的欄位:is_estimated×7」',
                ]
              },
            ]
          },
            ...
            blocks: [
              {
                ...
                items: [
                  '預設**摺疊**只佔一行,不擾動原有版面',
                  '摺疊狀態:「`⚠ N 個任務無今日資料`」吸引使用者點展開',
                  '展開後可單獨「重新整理」(不用 reload 整頁)',
                ]
              },
            ]
          },
          {
            type: 'tip',
            text: '🔍 **進階 debug**:在 admin「排程任務 → 執行歷史 → 詳情」可看完整 row errors 含 LLM 原始 payload(JSON 截 800 字),不用 SSH 進 K8s 看 log。'
          },
        ]
      },
        ...
        blocks: [
          {
            ...
            blocks: [
              {
                type: 'para',
                text: '左側篩選器 + 右側卡片列表。卡片版型(2026-04-28 改):'
              },
              {
                type: 'list',
                items: [
                  '**第一列(最顯眼)**:📅 日期(發佈日 / 抓取日雙標,過去 7 天 cutoff) · 來源(可點開原文,顯眼藍底) · 情緒色標 · 金屬 tag — 採購最關心「這篇是誰寫的、什麼時候、什麼金屬」',
                  '第二列:標題(粗體,點開原文)',
                  '第三列:中文摘要(80-150 字)',
                  '第四列:👍/👎 thumbs feedback + **[看完整內容]** 按鈕(2026-04-29 加)— 開 modal 並排「LLM 繁中摘要 + KB 原文 1500-3000 字」,不用點到外站才看得到全文',
                  '左側釘選按鈕保持(收藏)',
                ]
              },
              {
                type: 'note',
                text: '原版日期擠在卡片右上小字,使用者抱怨找不到 / 不顯眼。改完後日期 + 來源連結搬到左上,跟標題分離。**雙日期顯示**(發佈日 ≠ 抓取日時並列)讓使用者分辨「文章是 7 天前發的但今天才 RSS 推給我」這類情況。'
              },
              {
                type: 'table',
                headers: ['篩選器', '說明'],
                rows: [
                  ['📅 日期範圍',           '快捷:[今日][24h][7d][30d];也可手動輸入 from/to。預設 7 天 cutoff(對齊抓取 task 的 cutoff)'],
                  ['🪙 金屬(多選)',       '11 金屬 checkbox;預設套用「我的偏好」關注金屬'],
                  ['🌐 來源網站(多選)',    '從 pm_news url domain 反推(`?domain=mining.com` 直接 match URL)— 解決早期同網站不同 source 字串(如 `Mining.com` vs `mining.com news`)變體勾選不到的問題'],
          ...
          {
            type: 'subsection',
            title: '📊 歷史價格(Phase 5 D Track)',
            blocks: [
              {
                type: 'para',
                text: '採購每天最常看的 tab — **完整資料表 + 金屬價格 line chart**,還能疊「AI 預測線」「實際採購點」做擇時分析。'
              },
              {
                type: 'note',
                text: '🔀 **2026-04-28 順序變更**:資料表搬到上方優先看(包含當日全部欄位),圖表 + KPI 卡片移到下方。原版圖表在上,使用者要捲下去才看到表格。'
              },
              {
                type: 'subsection',
                title: '篩選器 + 圖表',
                blocks: [
                  {
                    type: 'list',
                    items: [
                      '📅 **日期範圍**:30d / 60d / 90d / 1y 快捷 + 手動 from/to',
                      '🪙 **金屬多選**:預設**全選 11 個**(沒設偏好時全部一起看);可手動勾選/清除',
                      '📈 **ECharts line chart**:每金屬一條線,X 軸鎖每日刻度(`MM-DD`),tooltip hover 顯示完整年月日 + 千分位數值',
                      '🔣 **Y 軸自動切 log**:>3 條線時自動切 log scale — 避免 PB(1.9k)被 SN(50k)壓成貼底平線(26 倍價差)',
                      '🎚️ **下方 dataZoom slider**:拖選局部區間放大;legend 移到頂端,不擋 X 軸 label',
                    ]
                  },
                ]
              },
              {
                type: 'subsection',
                title: '圖表疊加層(D — 採購節奏 vs 預測)',
                blocks: [
                  {
                    type: 'card_grid',
                    cols: 3,
                    items: [
                      { emoji: '🔮', title: 'AI 預測線(虛線)', desc: '從 forecast_history 撈 entity_type=metal 預測,虛線疊在歷史價格 line 上' },
                      { emoji: '🔵', title: '信心區間(80%)', desc: '半透明色塊填 [predicted_lower, predicted_upper]' },
                      { emoji: '🟢', title: '實際採購點 by 月', desc: '從 pm_purchase_history 撈每月平均單價;scatter 點大小 = 該月用量(8-30 px)' },
                    ]
                  },
                  {
                    type: 'note',
                    text: '若 forecast_history 無資料 → 提示「**預測線無資料**」(需先啟用 [PM] 每日金屬日報排程,或跑 `npm run pm:backfill-forecasts` backfill)。pm_purchase_history 無資料 → 提示「**需 ERP sync 啟用**」。'
                  },
                ]
              },
              {
                type: 'subsection',
                title: '三個 KPI 卡片(D Track)',
                blocks: [
                  {
                    type: 'table',
                    headers: ['卡片', '計算來源', '色標'],
                    rows: [
                      ['🎯 採購擇時(180 天)', '近 180 天「採購均單價 vs 市場月均」差異 %', '< 0% 綠(買在低點)、>0% 紅'],
                      ['🤖 AI 預測準度(30 天)', '近 30 天 pm_forecast_accuracy 平均 MAPE', '< 5% 綠 / 5-15% 黃 / > 15% 紅'],
                      ['📦 庫存週轉天數', '在庫+在途 ÷(月平均用量 / 30)', '< 30 天黃警告(短缺風險)'],
                    ]
                  },
                ]
              },
              {
                type: 'subsection',
                title: '完整資料表(下方 — 20 欄全資料)',
                blocks: [
                  {
                    type: 'list',
                    items: [
                      '所有 `pm_price_history` 欄位:as_of_date / metal_code / metal_name / original_price / original_currency / price_usd / unit / fx_rate_to_usd / day_change_pct / source / source_url / price_type / market / grade / lme_stock / stock_change / is_estimated / conversion_note / scraped_at',
                      '右上「⬇ 匯出此表 CSV」按鈕 — **client-side 直接從目前 rows 生 20 欄 CSV**(不打 API,Excel 開不亂碼)',
                    ]
                  },
                ]
              },
...
└──────────────┘         ┌─────────────┐         ┌─────────────┐   │     └──────────────┘
        ...
        blocks: [
          {
            ...
            cols: 2,
            items: [
              { emoji: '⏰', title: '1. 排程任務(2026-04-29 起 9 個 user-facing)', desc: '5 個 by-source 新聞 task + 全網報價 + 宏觀 + 日 / 週 / 月報,全部標 [PM] 前綴' },
  ...
  // ── 4. 資料流與每日週期 ────────────────────────────────────────────────────
  {
    ...
    blocks: [
      ...
      },
      {
        ...
        rows: [
          ['06:00', '`[PM] 全網金屬資料收集` 跑(Pro,純 11 金屬報價)', 'pm_price_history + PM-原始資料庫', '不用做'],
          ['08:30', '`[PM] 總體經濟指標日抓` 跑',                      'pm_macro_history',                 '不用做'],
          ['09:00', '`[PM] 新聞 Mining.com` 跑(15-20 篇)',            'PM-新聞庫',                        '不用做'],
          ['09:30', '`[PM] 新聞 Nikkei` 跑(5-10 篇)',                 'PM-新聞庫',                        '不用做'],
          ['09:30(週一/三)', '`[PM] 新聞 PGM評論` 跑(WPIC + JM)',  'PM-新聞庫',                        '不用做'],
          ['10:00', '`[PM] 新聞 SMM` 跑(中文中國)',                  'PM-新聞庫',                        '不用做'],
          ['10:30', '`[PM] 新聞 MoneyDJ` 跑(台灣財經)',              'PM-新聞庫',                        '不用做'],
          ['18:00', '`[PM] 每日金屬日報` 跑(統一綜合 + 新聞精選 5-10 篇 RAG + 預測)', 'pm_analysis_report + forecast_history + PM-分析庫 + **Email + docx**', '**收信看日報**(5 分鐘)'],
          ...
          ['每日 03:00(2026-04-29 加,可調)', '`pmRetentionCleanup` 跑 7 entity 過期清理', '砍 `pm_news` / KB chunks / `forecast_history` 等舊資料', '不用做(admin 在「PM 平台設定 → 資料保留設定」改天數)'],
        ]
      },
      {
        type: 'tip',
  ...
  // ── 5. 6 個排程任務說明 ───────────────────────────────────────────────────
  {
    ...
    last_modified: briefingLastModified,  // 2026-04-28:資料源變更(台銀 / JM RSS / Mining RSS 等)
    title: '6 個排程任務說明',
    sidebar_label: '排程任務',
    blocks: [
      {
      ...
      {
        type: 'subsection',
        title: '① 新聞抓取(2026-04-29 拆成 5 個 by-source task)',
        blocks: [
          {
            type: 'para',
            text: '**為什麼拆分**:原本單一「[PM] 每日金屬新聞抓取」task 一次跑 7 個 source,Flash 容易偷懶,常常某個 source 完全沒抓到、token 集中爆。**B+E 策略** = 每個 source 一個 task + 個別 prompt 專注該 source,加 `{{news_seen:source:days:limit}}` placeholder 讓 LLM 抓之前先比對已抓 url 主動跳過(取代純後端 dedupe)。'
          },
          {
            type: 'table',
            headers: ['Task', '排程', '資料源', '抓取數量', '備註'],
            rows: [
              ['`[PM] 新聞 Mining.com`',   '每日 09:00',         'Mining.com /feed/ RSS',          '15-20 篇',   '**主源** — RSS feed 36+ items 含完整內文'],
              ['`[PM] 新聞 Nikkei`',       '每日 09:30',         '日經中文 大宗商品',              '5-10 篇',    '政經視角(貿易戰 / 制裁 / 政策)'],
              ['`[PM] 新聞 SMM`',          '每日 10:00',         '上海有色網',                     '5-10 篇',    '中國市場視角(現貨價、產能、進出口)'],
              ['`[PM] 新聞 MoneyDJ`',      '每日 10:30',         'MoneyDJ 商品原物料',             '5-10 篇',    '台灣財經角度(LME 行情、台股供應鏈)'],
              ['`[PM] 新聞 PGM評論`',      '**週一 + 週三 09:30**', 'WPIC + matthey expert-insights', '1-5 篇',     '機構級評論,**60 天 cutoff**(quarterly / monthly 更新慢)'],
            ]
          },
          {
            type: 'table',
            ...
            rows: [
              ['模型',     'Flash(快、便宜)'],
              ['共用 pipeline', 'pm_news upsert by url_hash + KB:PM-新聞庫(content 強制 1500-3000 字原文)'],
              ['跨 task dedupe', '`{{news_seen:Mining.com:7:200}}` 等 placeholder — 把過去 N 天該 source 已抓 url 塞給 LLM,LLM 主動跳過'],
              ['cutoff',  '一般 7 天 / PGM 60 天(WPIC 季度、JM 月度發,7 天太嚴常空抓)'],
              ['email',   '不發信(2026-04-29 改 — email 由日報統一發)'],
              ['下游',     '日報統一從 PM-新聞庫 RAG 挑 5-10 篇精選 / Workflow news_agent 也走這個 KB'],
            ]
          },
          {
            type: 'note',
            text: '**KITCO Latest Metals News + MarketWatch 已驗證無法爬**(KITCO JS-rendered 沒 RSS、MarketWatch 對非登入回 401)。Mining.com RSS 涵蓋同類新聞作為替代。'
          },
          {
            type: 'tip',
            text: '🔧 採購若要新增第 6 個新聞源:複製 `buildNewsXxxTask` 範本 + 在 `_buildBySourceNewsPipeline` 共用 pipeline 下掛 + 加進 `BY_SOURCE_NEWS_TASKS` 註冊。**不要**把 prompt 又加回 master scrape — 那是純報價 task。'
          },
        ]
      },
      ...
      {
        type: 'subsection',
        title: '③ [PM] 全網金屬資料收集(2026-04-29 改純報價)',
        blocks: [
          {
            type: 'note',
            text: '🔄 **2026-04-29 重構**:原本同時抓報價 + 新聞,**砍掉新聞段**,職責改純抓 11 金屬當日報價。新聞職責整個搬給 ① 的 5 個 by-source task,避免「同篇文章被兩個 task 重複情緒打分」+「user 收兩封 email 混亂」。'
          },
          {
            type: 'table',
            headers: ['屬性', '值'],
            rows: [
              ['排程',     '每日 06:00(seed 預設;admin 可改)'],
              ['模型',     '**Pro**(多源差異綜合判斷)'],
              ['做什麼',   '抓 11 金屬當日報價(Au/Ag/Pt/Pd/Rh + Cu/Al/Ni/Sn/Zn/Pb),不抓新聞'],
              ['寫入',     '`pm_price_history`(11 筆 prices upsert by metal_code+as_of_date)+ KB:**PM-原始資料庫**(LLM 出 `_kb_doc` wrapper:今日報價快照當一份 KB doc 寫入,RAG 查得到「4/29 那天台銀金多少」)'],
              ['email',    '**不發信**(2026-04-29 改;報價快照不需 daily mail,日報會帶)'],
              ['下游',     '所有戰情 Design 的價格類圖表 / 歷史價格 tab / 報價 banner 都讀 pm_price_history'],
            ]
          },
          {
            type: 'subsection',
            title: '資料源 — 2026-04-28 採購需求變更',
            blocks: [
              {
                type: 'table',
                headers: ['金屬', '主源', '備註'],
                rows: [
                  ['Cu / Al / Ni / Sn / Zn / Pb(基本金屬)', 'Westmetall(LME cash)', 'scrape;含庫存 + 庫存變動'],
                  ['**AU 金**', '**台灣銀行黃金存摺**', '**fetch**(USD 黃金存摺賣出價,1英兩=1 troy oz)'],
                  ['**AG 銀**', 'Westmetall(主)+ Kitco(備援)', 'Westmetall EUR/kg → USD/oz 換算;Kitco 直接 USD/oz'],
                  ['**PT / PD / RH**', '**Johnson Matthey RSS**', '**fetch**(`matthey.com/pgm-prices/rss-feed.xml`,JM Base Price 業界基準)'],
                  ['Pt/Pd/Rh `day_change_pct`', 'TradingEconomics', '補日變動 %(JM RSS 沒帶)'],
                ]
              },
              {
                type: 'note',
                text: '為何這樣選:正崴採購黃金透過台銀做交易/避險,**台銀牌價(含 spread)就是實際結算價**;PGM 採購多以 **JM Base Price 為合約結算基準**。LBMA Gold/Silver 已將資料權獨家授權 ICE/IBA 付費,公開網頁無法爬。'
              },
            ]
          },
          {
            type: 'tip',
            text: '為什麼用 Pro 而不是 Flash:**多源差異告警需要綜合判斷**(例如「台銀比 Westmetall 高 0.5% 是正常匯差還是異常」),Flash 容易誤報。'
          },
        ]
      },
      ...
      {
        type: 'subsection',
        title: '④ [PM] 每日金屬日報(2026-04-29 統一收口)',
        blocks: [
          {
            type: 'note',
            text: '🔄 **2026-04-29 重構**:成為 PM 平台**唯一**對採購發每日 email 的 task。新增「🔥 今日重要新聞精選 5-10 篇」段(從 PM-新聞庫 RAG 挑採購最在意的);保留 forecast_history + pm_analysis_report + DOCX 全鏈路。'
          },
          {
            type: 'table',
            headers: ['屬性', '值'],
            rows: [
              ['排程',     '每日 18:00(在 ① by-source 新聞 + ② 宏觀 + ③ 報價之後跑,所有素材已備齊)'],
              ['模型',     'Pro(寫報告,需要細緻語言)'],
              ['做什麼',   '讀 PM-新聞庫(RAG 挑 5-10 篇精選新聞)+ PM-原始資料庫(報價快照)+ 宏觀,LLM 寫完整日報(含「為什麼今日金價這樣動」+ 7 天預測 + 採購建議)'],
              ['輸出',     '① pm_analysis_report(report_type=daily)<br/>② forecast_history(7 天 mean / lower / upper)<br/>③ 寫進 PM-分析庫(供週/月報 RAG)<br/>④ `金屬市場日報_{{date}}.docx`<br/>⑤ **Email 採購群組**(新聞精選 + 報告附件)'],
              ['模板',     '見「文件模板」章節 → 日報模板'],
            ]
          },
          {
            type: 'tip',
            text: '🔥 「新聞精選」從 PM-新聞庫挑時 prompt 強調「採購最在意的供需 / 政策 / 庫存類」,不是綜合性新聞。觀察區間 24-48h,情緒 negative 優先。'
          },
        ]
      },
  ...
  // ── 5.5. 資料保留設定(2026-04-29 新增)──────────────────────────────────
  {
    id: 'pm-data-retention',
    sort_order: 55,
    icon: 'Eraser',
    icon_color: 'text-amber-600',
    last_modified: '2026-04-29',
    title: '資料保留設定(retention)',
    sidebar_label: '資料保留設定',
    blocks: [
      {
        type: 'para',
        text: '為避免 `pm_news` / KB chunks 無限累積撐爆 Oracle / 拖慢檢索,**每個資料 entity 各自設天數**;留空 = 永久保留。**Admin 在「系統管理 → PM 平台設定 → 資料保留設定」** section 統一管理。'
      },
      {
        type: 'subsection',
        title: '7 個 entity 預設保留天數',
        blocks: [
          {
            type: 'table',
            headers: ['Entity', '預設天數', '為什麼'],
            rows: [
              ['`pm_news` + KB:PM-新聞庫',                '180',     '同步清(綁同一天數)避免 user 在 briefing 點「看完整內容」找不到 KB 全文'],
              ['KB:PM-原始資料庫',                        '14',      '報價快照只看「最近怎麼動」,留 14 天足夠(歷史價格走 pm_price_history)'],
              ['KB:PM-分析庫 + `pm_analysis_report`',     '**永久**', 'LLM 日 / 週 / 月報是趨勢分析的歷史軌跡,不能砍'],
              ['`pm_price_history`',                      '**永久**', '所有戰情 line chart 的根基,砍了趨勢圖就沒'],
              ['`pm_macro_history`',                      '**永久**', 'DXY / VIX / 10Y 對金屬影響需長期觀察'],
              ['`forecast_history`',                      '90',      'MAPE 校驗最多看 30-60 天,90 天緩衝足夠'],
              ['`pm_alert_history`',                      '180',     '半年警示記錄,事後追溯用'],
            ]
          },
        ]
      },
        ...
        blocks: [
          {
            type: 'list',
            items: [
              '**daily 凌晨 3:00 預設跑**(避開白天的 [PM] 排程)',
              '排程時間可在 UI 改:「啟用 / 時 / 分」三個欄位 → 套用後 server **立即 reload cron**(不用重啟)',
              '想完全停用:把「啟用」勾掉',
              '裝置 / 手動執行:UI 有「**立即執行清理**」按鈕(confirm 後跑,顯示 result summary),不等排程',
            ]
          },
        ]
      },
        ...
        blocks: [
          {
            type: 'steps',
            items: [
              { title: '進 AdminDashboard → 系統管理 → PM 平台設定',                              desc: '找到「**資料保留設定**」section' },
              { title: '看每個 entity 表格列:總筆 / 最舊日期 / 會清掉幾筆(dry-run)',          desc: '不寫入,只計算' },
              { title: '改右側「保留天數」input(1-3650 天有效;留空 = 永久)',                  desc: '改完按「儲存」' },
              { title: '想立即清理 → 按「立即執行清理」按鈕',                                    desc: '會跑同一邏輯但實際 DELETE,顯示「砍了 N news + M kb_chunks + ...」result' },
              { title: 'KB 類連動清:kb_chunks + kb_documents + 重算 knowledge_bases stats',     desc: '不會孤兒,UI 立即看到 KB 文件數下降' },
            ]
          },
          {
            type: 'tip',
            text: '🔍 **怎麼知道清掉了**:UI 表格上方「最舊日期」行會立即往新移;對應 KB 詳情頁的「文件數 / chunks 數」也會下降。Audit 可看 `pm_retention_log`(若啟用)。'
          },
        ]
      },
      {
        type: 'note',
        text: '⚠️ **永久保留的東西不會被清**:即使你誤填「`pm_price_history` 7 天」,後端 hardcoded 防護會拒絕(這幾個 entity 在 `RETENTION_PROTECTED` 集合中)。改要砍價格歷史 → 走 DBA 手動 truncate。'
      },
    ]
  },
