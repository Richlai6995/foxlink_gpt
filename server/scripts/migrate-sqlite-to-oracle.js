...
/**
 * Insert rows; skip on ORA-00001 (duplicate key).
 * Schema uses GENERATED AS IDENTITY (= BY DEFAULT), so explicit id values work without OVERRIDING.
 */
async function mergeInsert(pool, table, cols, rows) {
  if (!rows.length) return 0;
  let inserted = 0;
  const conn = await pool.getConnection();
  try {
    const bindNames = cols.map((_, i) => `:${i + 1}`);
    const sql = `INSERT INTO ${table} (${cols.join(', ')}) VALUES (${bindNames.join(', ')})`;
    ...
    for (const row of rows) {
      ...
      try {
        ...
        if (e.errorNum === 1)    continue; // ORA-00001: duplicate key — already exists
        if (e.errorNum === 2291) continue; // ORA-02291: FK violation — orphaned row in SQLite (FKs not enforced there)
        throw e;
      }
    }
  } finally {
...
async function main() {
  ...
  // ── Prep: change GENERATED ALWAYS → BY DEFAULT so we can insert explicit IDs ──
  const IDENTITY_TABLES = [
    'roles','users','llm_models','chat_messages','token_usage','token_prices',
    'audit_logs','sensitive_keywords','password_reset_tokens',
    'scheduled_tasks','scheduled_task_runs',
    'mcp_servers','mcp_call_logs','dify_knowledge_bases','dify_call_logs','skills',
  ];
  console.log('\n[Prep] Switching identity columns to BY DEFAULT...');
  for (const t of IDENTITY_TABLES) {
    try {
      await oracleExec(pool,
        `ALTER TABLE ${t} MODIFY (id NUMBER GENERATED BY DEFAULT AS IDENTITY)`);
      process.stdout.write(` ${t}`);
    } catch (e) {
      console.warn(`\n[Prep] ALTER ${t}: ${e.message}`);
    }
  }
  console.log('\n[Prep] Done.');
  ...
  // ── Post-migration: reset sequences so next AUTO id starts above max migrated id ──
  console.log('\n[Post] Resetting identity sequences to START WITH LIMIT VALUE...');
  for (const t of IDENTITY_TABLES) {
    try {
      await oracleExec(pool,
        `ALTER TABLE ${t} MODIFY (id NUMBER GENERATED BY DEFAULT AS IDENTITY (START WITH LIMIT VALUE))`);
      process.stdout.write(` ${t}`);
    } catch (e) {
      console.warn(`\n[Post] ALTER ${t}: ${e.message}`);
    }
  }
  console.log('\n[Post] Done.');
