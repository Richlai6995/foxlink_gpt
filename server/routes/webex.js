...
const WEBEX_I18N = {
  ...
  not_found: (email) => [
    ...
    `1. You have not logged into the web system to create an account. Please log in via the company intranet first: https://flgpt.foxlink.com.tw`,
    ...
    `1. Bạn chưa đăng nhập vào hệ thống web để tạo tài khoản. Vui lòng đăng nhập qua mạng nội bộ công ty trước: https://flgpt.foxlink.com.tw`,
...
async function findUserByEmail(db, rawEmail) {
  ...
  if (row) {
    console.log(`[Webex][Auth] ✅ user found: id=${row.id} username="${row.username}" email="${row.email}" status=${row.status}`);
  } else {
