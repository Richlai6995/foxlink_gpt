...
router.get('/sessions/:id', async (req, res) => {
  try {
    ...
    messages.forEach((m) => {
      ...
      // Chat Inline Chart:重開對話時還原 charts 陣列給前端 render
      if (m.charts_json) {
        try {
          const arr = JSON.parse(m.charts_json);
          if (Array.isArray(arr) && arr.length > 0) m.charts = arr;
        } catch (e) { }
      }
    });
...
router.post('/sessions/:id/messages', uploadChatFiles, budgetGuard, async (req, res) => {
  ...
  try {
    ...
    // pureMode 必須 short-circuit — 不論前端送什麼 erp_tool_ids 都當沒看到
    let topbarErpSkills = [];
    if (!pureMode && Array.isArray(userErpToolIds) && userErpToolIds.length > 0) {
      try {
    ...
    if (hasXlsxAttached) {
      ...
      if (excelSkillInjected) {
        ...
        skillSystemPrompts.push(
          ...
          `6. 若使用者問題模糊(例如「分析這個檔」),先呼叫 excel_query 跑 \`DESCRIBE t\` 看 schema,再決定後續查詢。\n` +
          multiFileHint
        );
      } else {
    ...
    if (hasPdfAttached) {
      ...
      if (pdfToDocxSkillInjected) {
        skillSystemPrompts.push(
          `# ⚠️⚠️⚠️ PDF 附件處理規則 — 本段為「規則覆蓋」,優先級高於全局系統指令\n\n` +
          `本對話偵測到 ${pdfFiles.length} 個 PDF 附件,且 \`pdf_to_docx\` 工具**已注入到你的 tools 清單**(可直接 function-call)。\n\n` +
          `**全局系統指令裡「使用者要求轉換就寫 \`\`\`generate_docx\`\`\` 代碼塊」的規定,在本對話完全失效。**\n` +
          `若你寫 \`\`\`generate_docx\`\`\` 代碼塊,**伺服器後處理會自動 strip 掉**,使用者拿到 0 個 docx 檔,並收到系統警告訊息要求你重做。\n\n` +
          `## 可用檔案\n${pdfFileList}\n\n` +
          `## 強制規則\n` +
          `1. 使用者任何 PDF → DOCX 轉檔需求(「轉成 Word / 轉 docx / 轉檔可編輯 / 轉成 Word 檔 / 用 vision mode 轉 / 把 PDF 弄成 Word」),**只能透過 function-call 呼叫 \`pdf_to_docx\` 工具**,沒有任何例外。\n` +
          `2. **嚴格禁止** \`\`\`generate_docx:filename\`\`\` 代碼塊 — inline PDF 看到的內容是你看圖的「視覺解讀」,不是真檔案結構;自寫 markdown 會丟失表格底色、合併儲存格、欄寬、列高、嵌入圖、字型,使用者拿到的是「AI 重打的近似版」而非「真轉檔」。\n` +
          `3. 呼叫 \`pdf_to_docx\` 時:\n` +
          `   - \`file_name\` 從上方清單選(只一份 PDF 可省略)\n` +
          `   - \`format\` 預設 \`'auto'\`(系統依 complexity_score 自動選);使用者說「保留格式 / 表格底色 / 複雜版面 / vision / 高品質」→ 直接帶 \`format='vision'\`\n` +
          `   - 工具回應含 \`complexity_score\`(0-100)+ \`recommended_mode\`;score≥30 用 editable 通常會跑版,主動建議改 vision 重做\n` +
          `4. 加密 PDF:先不帶 \`password\` 呼叫,系統回 \`PASSWORD_REQUIRED\` → 請使用者提供密碼後帶入重呼。\n` +
          `5. 工具回 \`files: [{type:'docx', publicUrl}]\`,前端會自動顯示下載連結,你只需簡短說明結果(模式 / 頁數 / 耗時),**不要在 chat 內貼 markdown 下載連結**(重複)。\n` +
          `6. 工具失敗(MAX_TOKENS / vision 失敗 / convert error)→ 如實轉達 + 建議使用者改 \`vision_model='pro'\` 或切頁;**絕不 fallback 用 \`\`\`generate_docx\`\`\` 假轉,寧可說「轉檔失敗」也不要產生假結果**。\n` +
          `7. 使用者問 PDF 內容(摘要、找特定資訊)→ 直接基於 inline PDF 回答,**不需呼叫 \`pdf_to_docx\`**(那是「轉檔」工具)。`
        );
      } else {
...
- 未標記保留原文的欄位：直接提取文字中的資料，不要杜撰`;
    ...
    // 優先序:X-Lang header (UI 當前語言) > users.preferred_language (DB 設定) > zh-TW
    // 這樣使用者切 UI 語言 toggle 時,AI 回覆立即跟著切,不用改 profile
    const headerLang = (req.headers['x-lang'] || req.headers['X-Lang'] || '').toString();
    const validLangs = ['zh-TW', 'en', 'vi'];
    const resolvedLang = (validLangs.includes(headerLang) ? headerLang : null)
                      || _userProfile?.preferred_language
                      || 'zh-TW';
    console.log(`[Chat] resolvedLang=${resolvedLang} (X-Lang="${headerLang}", DB="${_userProfile?.preferred_language}")`);
    const LANG_NAMES = { 'zh-TW': '繁體中文', 'en': 'English', 'vi': 'Tiếng Việt' };
    const langName = LANG_NAMES[resolvedLang] || '繁體中文';
    // 強制以使用者 UI 設定的語言回答，不因為 input/KB 內容是別的語言就漂移。
    // 只在「明確翻譯請求」（如「翻成日文」「translate to French」）才切換。
    const langInstruction =
      `\n\n---\n` +
      `【輸出語言】必須使用「${langName}」回答所有內容。\n` +
      `重要規則：\n` +
      `- 即使使用者輸入、知識庫內容、附件是其他語言（如中文、英文），你的回答仍必須是「${langName}」。\n` +
      `- 專有名詞（人名、產品型號、技術術語）可保留原文。\n` +
      `- 唯一例外：使用者明確要求翻譯成特定語言（如「翻成日文」「translate to French」），才依使用者指定的語言輸出。\n` +
      `- 不要因為輸入語言是中文就自動切換回中文。`;
    // ── Chat Inline Chart:教 LLM 輸出 ```generate_chart:type``` 代碼區塊 ──
    // 詳見 docs/chat-inline-chart-plan.md §4.6
    // 刻意不依賴 wantsFileGen flag — 任何路徑都該認得 chart fence
    const chartInstruction =
      `\n\n---\n` +
      `【圖表視覺化能力】當你的回答包含可視化的數值資料(時間序列、分類比較、比例分布),且符合下列條件時,請輸出圖表區塊:\n` +
      `- 資料筆數 3 ~ 100 rows 之間(太少沒意義、太多改用表格)\n` +
      `- 時間序列選 line;分類比較選 bar;比例選 pie;多系列比較選 bar 或 line\n` +
      `- 來自 tool / MCP / ERP 結果時,優先用 data_ref 指向結果,不要複製 rows(節省 token)\n` +
      `\n格式(放在回答末段,可多張):\n` +
      '```generate_chart:bar\n' +
      `{\n` +
      `  "title": "標題",\n` +
      ...
      `  "data": [{"site": "龜山", "output": 12500}, {"site": "林口", "output": 9800}]\n` +
      `}\n` +
      '```\n' +
      `\n大資料 (data_ref) 範例 — tool_call_id 填「工具名稱」,path 為 JSONPath:\n` +
      '```generate_chart:line\n' +
      `{\n` +
      `  "title": "近 30 天訂單趨勢",\n` +
      `  "x_field": "date",\n` +
      `  "y_fields": [{"field": "count"}],\n` +
      `  "data_ref": {"tool_call_id": "getOrderDaily", "path": "$.rows"}\n` +
      `}\n` +
      '```\n' +
      `\n進階圖型(僅在資料明確符合形態時用):\n` +
      `- scatter:散點圖,x_field + y_fields[0] 都是數值,看相關性\n` +
      `- heatmap:熱力圖,y_fields 必須給 2 個(group 縱軸 + value 顏色),3 維資料才用\n` +
      `- radar:雷達圖,x_field 是 indicator name 欄,y_fields 各為一個系列(多人 / 多廠比較多面向)\n` +
      `\n規則:\n` +
      `- type 支援:bar / line / pie / area / scatter / heatmap / radar\n` +
      `- 一個回答可以有多張圖,每張用獨立代碼區塊\n` +
      `- 不要畫:純文字答覆、單一數字、描述性內容、raw 文件摘要\n` +
      `- 資料超過 200 筆會自動降採樣,但仍以 100 rows 內為佳\n` +
      `- 超過 30 點會自動掛 dataZoom,使用者可拖曳/滾輪縮放\n` +
      `- 不確定就不畫,寧可漏畫不要亂畫`;
    ...
    if (imageOutput) {
      ...
      // 圖片生成路徑也要傳 langInstruction,否則 AI 用圖片模式仍會漂中文
      imgResult = await generateWithImage(apiModel, history, userParts, langInstruction);
      text = imgResult.text;
      ...
      let mcpServerInstructionsMap = {};
      let allDeclarations = [];

      if (pureMode) {
        // ── 純對話模式 — 完全 skip MCP/KB/Dify loading,allDeclarations 留空 ──
        // 即使前端送了 mcp_server_ids/self_kb_ids/dify_kb_ids,也當沒看到
        console.log('[Chat] Pure mode → skip MCP/Dify/SelfKB explicit loading, allDeclarations stays empty');
        ...
        let _idx = 0;
        if (hasMcp) {
          const { functionDeclarations: allMcpDecls, serverMap: sm, serverInstructionsMap: sim } = _explicitResults[_idx++];
          serverMap = sm;
      ...
      if (allDeclarations.length > 0) {
        ...
        if (pureDify) {
          ...
          const finalInstruction = [skillExtraInstruction, templateExtraInstruction, difyInstruction, langInstruction, chartInstruction].filter(Boolean).join('\n\n---\n\n');
          ...
          const finalInstruction = [skillExtraInstruction, templateExtraInstruction, kbInstruction, langInstruction, chartInstruction].filter(Boolean).join('\n\n---\n\n');
          let firstChunkReceived = false;
          const keepAliveInterval = setInterval(() => {
          ...
          // 訊息要反映實際載入的東西。code skill 是獨立計數,MCP/KB 為 0 時別嚇使用者。
          const codeSkillCount = Object.keys(codeSkillToolMap).length;
          const _parts = [];
          if (mcpCount > 0)       _parts.push(`${mcpCount} 個 MCP 工具`);
          if (kbTotal > 0)        _parts.push(`${kbTotal} 個知識庫`);
          if (codeSkillCount > 0) _parts.push(`${codeSkillCount} 個技能`);
          if (_parts.length > 0) {
            sendEvent({ type: 'status', message: `已載入 ${_parts.join('、')}` });
          }

          const calledDifyKbs = new Set();
          const calledSelfKbs = new Set();

          const _rawToolHandler = async (toolName, args) => {
            ...
            if (codeSkillToolMap[toolName]) {
              ...
              if (isExcelQuerySkill(sk)) {
                ...
                // PDF 不能走 Excel pipeline — XLSX.readFile 會把它解析成 40000+ 列垃圾表,
                // 然後 DuckDB load 卡 event loop > 4 分鐘觸發 K8s liveness fail(2026-06-02 事故)
                if (target.type !== 'xlsx') {
                  const xlsxList = sessionAttachedFiles.filter(f => f.type === 'xlsx');
                  return `[Excel 查詢失敗] "${fileName}" 不是 Excel 檔(type=${target.type || 'unknown'}),excel_query 只支援 .xlsx/.xls。可用 Excel:\n${xlsxList.length ? xlsxList.map((f, i) => `${i+1}. ${f.name}`).join('\n') : '(無)'}`;
                }

                try {
                  // 把 session 內所有「非主檔」的 xlsx 當作 extraFiles 一起 load 進 DuckDB,
                  // 讓 LLM 一個 SQL 就能 cross-file JOIN(e.g. 「比較兩個 BOM 差異」這種 task)
                  // 主檔 → t,extras → f1, f2, ...,system prompt 教 LLM 怎麼用
                  // PDF/其他類型不要塞進來(同上,會被 XLSX 強解成爛表)
                  const extraFiles = sessionAttachedFiles
                    .filter(f => f.path !== target.path && f.type === 'xlsx')
              ...
              try {
                ...
                // Phase 1c:skill 偵測加密 PDF → 推自訂 SSE event 給前端跳 password modal
                // (密碼從未進 chat content;前端 modal 直接 POST /api/pdf-docx-jobs/decrypt-submit)
                if (data.pdf_password_prompt && data.pdf_password_prompt.token) {
                  console.log(`[Chat] pdf_password_prompt SSE event sent: token=${String(data.pdf_password_prompt.token).slice(0, 8)} file=${data.pdf_password_prompt.file_name}`);
                  sendEvent({
                    type: 'pdf_password_prompt',
                    token: data.pdf_password_prompt.token,
                    file_name: data.pdf_password_prompt.file_name,
                    expires_in: data.pdf_password_prompt.expires_in,
                    session_id: sessionId,
                  });
                }
            ...
            // Answer 模式 + passthrough 命中:artifact 已 SSE 直送使用者,
            // 不能再把 summaryText(給 LLM 看的 metadata)塞進回覆 — 會重複醜文字
            if (ptResult !== null && srv.response_mode === 'answer') return '';
          ...
          // MCP ServerInstructions:只對實際進入 allDeclarations 的 MCP server 注入
          const mcpInstruction = mcpClient.buildMcpSystemInstructions(
            serverMap, allDeclarations.map(d => d.name), mcpServerInstructionsMap
          );
          if (mcpInstruction) {
            console.log(`[Chat] Injecting MCP server instructions, length=${mcpInstruction.length}`);
          }
        ...
        try {
          ...
          // 加 langInstruction 確保標準文字路徑也依 UI 語言回
          const combinedInstruction = [skillExtraInstruction, templateExtraInstruction, langInstruction, chartInstruction].filter(Boolean).join('\n\n---\n\n');
          if (providerType === 'azure_openai' && modelRow) {
      ...
      // ── 防線:有 PDF + pdf_to_docx skill 啟用時,strip 掉 LLM 私自寫的 generate_docx 假轉 ──
      // 即使 system prompt 禁止 + skill 被注入,Gemini 3 Pro 強訓練的 generate_docx pattern
      // 加上 inline PDF 看得到內容,仍可能不聽指令自己寫 markdown 組 fake docx。
      // 這層 deterministic 攔截保證使用者不會拿到「LLM 重打的假轉檔」。
      if (hasPdfAttached && pdfToDocxSkillInjected) {
        const docxBlockRe = /```generate_docx[^\n]*\n[\s\S]*?```/g;
        const docxMatches = text.match(docxBlockRe);
        if (docxMatches && docxMatches.length > 0) {
          console.warn(`[Chat] PDF attached + pdf_to_docx injected, but LLM wrote ${docxMatches.length} generate_docx block(s) — stripping (likely fake conversion bypassing skill)`);
          text = text.replace(docxBlockRe, '');
          const warning =
            '\n\n⚠️ **系統攔截**:AI 嘗試用 markdown 代碼塊自寫 docx(看似 PDF 轉檔但實為 AI 重打的內容)。' +
            '本對話有 PDF 附件且 `pdf_to_docx` 工具已啟用,真正的 PDF 轉檔必須走工具。' +
            '請對 AI 說「**用 pdf_to_docx 工具轉檔,format=vision**」,我會用 Gemini Vision 重組真正的 docx。\n';
          sendEvent({ type: 'chunk', content: warning });
          // 重算 blockHeaders 讓下面狀態訊息準確
          const newHeaders = text.match(/```generate_\w+:[^\n]+/g);
          if (newHeaders) blockHeaders.length = newHeaders.length;
          else blockHeaders && (blockHeaders.length = 0);
        }
      }
      ...
      try {
        ...
        console.error('[Chart] parseChartBlocks failed:', e.message);
      }
