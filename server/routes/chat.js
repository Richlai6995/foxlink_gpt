...
router.post('/sessions/:id/messages', upload.array('files', 10), async (req, res) => {
  ...
  try {
    ...
    const wantsFoxlinkPpt = !docTemplateId && isPptRequest &&
    ...
    if (docTemplateId && docTemplateSchema) {
      ...
      const pptxHint = isPptxLayout
        ? `\n5. PPTX 版型：slides 欄位是陣列，每個元素用 "type":"bullets" 或 "type":"3col"，內容以 \\n 分隔每條重點，合理拆成多張投影片`
        : '';
      userParts.push({
    ...
    if (imageOutput) {
      ...
      if (docTemplateId && docTemplateSchema) {
        try {
          ...
          // P0: PPTX Layout Engine (only for pptx layout_template schemas)
          const isPptxLayout = docTemplateSchema.pptx_settings?.slide_config?.some(c => c.type === 'layout_template');
          if (isPptxLayout && Array.isArray(inputData.slides) && inputData.slides.length > 0) {
