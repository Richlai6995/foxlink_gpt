...
| `cortex-cost-model-import-plan.md` | **成本模型通用匯入規劃(C 系列 · 已拍板待實作)**:廠別標準(baseline/IDL/耗材)vs 案級調整 兩層;單一 Excel 多分頁(costing_model 決定必要頁);範本庫=系統保留「範本專案」(fixture 轉正);薪資口徑=月薪→時薪換算(匯入層算,引擎不改);切片 C-1(直匯專案+範本下載)→C-2(範本庫)→C-3(版本/權限)。BOM 架構完成後開工 |
