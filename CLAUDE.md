...
| `cortex-bom-source-excel-structure.md` | **⭐ Cortex BOM 來源 Excel 架構解剖(SOT)**:WHOOP / Rival3 兩份真實報價 Excel 的權威結構紀錄。**所有 BOM 規劃(變異維度/成本模型/匯入 profile/config/多廠對比)都以此為前提,不再重述**。兩份本質都是 configurable BOM(超級 BOM);報價 = config(顏色×包裝)× 廠別(CN/VN/TW)× 數量 矩陣。EE 共用/ME 分色/PKG 分包裝 = 料層 effectivity。Golden:WHOOP 89.5537、Rival3 CN Black 11.123/White 11.338 |
| `cortex-cost-model-import-plan.md` | **成本模型通用匯入規劃(C 系列 · 已全數 ship)**:廠別標準 vs 案級 兩層;單一 Excel 多分頁;範本庫=系統範本專案 CORTEX-COST-TPL(版本化/停用);薪資=月薪→時薪換算;round-trip 六位等值(SIMPLIFIED 106.286214 / FULL 12.027039) |
| `cortex-roadmap.md` | **⭐ Cortex Roadmap(下一步規劃唯一紀錄處)**:已完成全覽(統一格式 v2/變異軸/FLK 層級/矩陣/C 系列/全資料隨檔/DEMO 包)+ Backlog 優先序:P1=W4 v0.12 gap(開案wizard/Stage Gate/機密遮罩S2/議價/報價PDF/AI比對)· P2=範本庫管理頁+Profile UI · P3=B-4 加權/per-factory 料價/B-6 ERP帶價/EPM權限/i18n · 技術債(bind lint/fixture退場/dark-launch出場) |
